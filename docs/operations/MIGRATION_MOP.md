...
### 4.2 Red Hat 3scale (Operator API Manager Database)
3scale configurations are fully stored in a system database.

1. **Dump 3scale System Database (on-premise Postgres pod)**:
   ```bash
   oc exec -it -n 3scale deploy/system-postgresql -- pg_dump -U postgres -d system -F c -f /tmp/3scale-system.dump
   oc cp -n 3scale deploy/system-postgresql:/tmp/3scale-system.dump ./3scale-system.dump
   ```
2. **Upload to S3**:
   ```bash
   aws s3 cp 3scale-system.dump s3://payu-migration-backup-787842753050/3scale/
   ```
3. **Restore to target database (in-cluster target or RDS target)**:
   * Deploy the 3scale Operator manifests on `payu-prod`.
   * Apply database schemas, restore the PostgreSQL dump, and spin up the APIManager CR using the target DB configuration.
