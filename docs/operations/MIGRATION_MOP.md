...
### 4.2 Red Hat 3scale (Operator API Manager Migration MOP)
*This section defines the Method of Procedure (MOP) for migrating the Red Hat 3scale API Management platform (v2.15) from payu-onprem to payu-prod using the Operator-based fresh deployment and database synchronization.*

#### System Configuration Overview:
* **3scale Version**: `2.15` (Operator-installed).
* **Source/Target Deployment**: Kubernetes Operator-managed API Manager.
* **Automation Level**: 100% manual work (no automation tools).
* **Domain Access Mode**: Changes exist between `payu-onprem` and cloud domain names. Developer portal URL, admin dashboard, and APIcast wildcard routes must be updated to the new DNS domains.
* **Custom Plugins**: None (vanilla 3scale installation).
* **Traffic Control Invariant**: All traffic to the cluster (Inbound, Internal, Outbound) is intercepted and routed through the 3scale APIcast gateway.
* **Traffic Types Handled**:
  1. **Inbound Traffic**: Traffic from external consumers/clients to internal cluster workloads.
  2. **Internal Traffic**: Internal service-to-service communications routed through APIcast for authorization, usage monitoring, and rate-limiting.
  3. **Outbound Traffic**: Egress traffic originating from internal cluster workloads to external partner systems (escrow accounts, TokoBapak, Nobar, banks).
* **Authentication Integrations**: Multiple authentication modes:
  - **API Keys**: Direct client token validation by APIcast against the system database.
  - **JWT Tokens**: OpenID Connect (OIDC) JWT tokens generated by the Red Hat Single Sign On (RH-SSO) instance on AWS. APIcast decrypts and validates token signatures before passing requests downstream.

#### 4.2.1 Phase 1 — Target Environment & Operator Provisioning
1. **Deploy 3scale Operator on payu-prod**:
   * Navigate to OperatorHub, search for **Red Hat 3scale 2.15**, and deploy the subscription inside the namespace `3scale` (or the namespace managing API gateways).
2. **Configure Target Secrets**:
   * Prepare external database secrets (`system-database` and Redis secrets) for postgres and Redis backends on AWS.
3. **Provision DNS Wildcards**:
   * Setup AWS Route 53 A-alias records to map `*.apps.payu-prod.payu.ocp.fajjjar.my.id` to the APIcast ingress load balancers.

#### 4.2.2 Phase 2 — Database Backup (PostgreSQL & Redis)
1. **Stop ingress traffic and active logins**:
   * Scale down the client-facing APIcast deployments on `payu-onprem` to `0` to prevent write drifts during backup:
     ```bash
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig scale deployment/apicast-production -n 3scale --replicas=0
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig scale deployment/system-app -n 3scale --replicas=0
     ```
2. **Dump 3scale System Database (on-premise PostgreSQL pod)**:
   * Execute `pg_dump` on the Postgres pod inside the source cluster:
     ```bash
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig exec -it -n 3scale deploy/system-postgresql -- pg_dump -U postgres -d system -F c -f /tmp/3scale-system.dump
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig cp -n 3scale deploy/system-postgresql:/tmp/3scale-system.dump ./3scale-system.dump
     ```
3. **Backup Redis States**:
   * For the two Redis instances (system-redis and backend-redis), run `SAVE` or grab the latest dump `.rdb` files from their storage mounts to migrate runtime token caches and rate-limit statistics:
     ```bash
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig exec -it -n 3scale deploy/system-redis -- redis-cli SAVE
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig cp -n 3scale deploy/system-redis:/data/dump.rdb ./3scale-system-redis.rdb
     
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig exec -it -n 3scale deploy/backend-redis -- redis-cli SAVE
     oc --kubeconfig ~/.kube/payu-onprem.kubeconfig cp -n 3scale deploy/backend-redis:/data/dump.rdb ./3scale-backend-redis.rdb
     ```
4. **Upload backups to AWS S3**:
   ```bash
   aws s3 cp 3scale-system.dump s3://payu-migration-backup-787842753050/3scale/
   aws s3 cp 3scale-system-redis.rdb s3://payu-migration-backup-787842753050/3scale/
   aws s3 cp 3scale-backend-redis.rdb s3://payu-migration-backup-787842753050/3scale/
   ```

#### 4.2.3 Phase 3 — Database Restore and Domain Domain Adjustment
1. **Restore PostgreSQL System DB**:
   * Download the postgres dump from S3 and restore it to the target PostgreSQL database in the cloud:
     ```bash
     pg_restore -h <target-cloud-db-host> -U postgres -d system -v ./3scale-system.dump
     ```
2. **Copy Redis dump `.rdb` files into target PVCs**:
   * Prior to spinning up Redis pods on `payu-prod`, mount the persistent volumes and copy `3scale-system-redis.rdb` and `3scale-backend-redis.rdb` as `/data/dump.rdb` to initialize them with the correct session states.
3. **Execute Domain Update Queries in PostgreSQL**:
   * Connect to the restored database and update the account/portal domain mappings from the old `payu-onprem` domains to the new cloud domains:
     ```sql
     UPDATE accounts SET self_domain = REPLACE(self_domain, 'apps.payu-onprem.payu.ocp.fajjjar.my.id', 'apps.payu.ocp.fajjjar.my.id') WHERE self_domain LIKE '%apps.payu-onprem%';
     UPDATE accounts SET support_email = 'platform-engineering@payu.ocp.fajjjar.my.id';
     -- Also update domain routing tables
     UPDATE routes SET host = REPLACE(host, 'payu-onprem', 'payu') WHERE host LIKE '%payu-onprem%';
     ```

#### 4.2.4 Phase 4 — APIManager CR Deployment & DNS Update
1. **Apply the APIManager Custom Resource**:
   * Create and apply the `APIManager` CR yaml, referencing the target domain wildcards and mapping external database secrets:
     ```yaml
     apiVersion: apps.3scale.net/v1alpha1
     kind: APIManager
     metadata:
       name: payu-apimanager
       namespace: 3scale
     spec:
       wildcardDomain: apps.payu.ocp.fajjjar.my.id
       databaseConnectionString: postgresql://postgres:password@target-db-host:5432/system
       # Configure S3 storage, Redis configuration, and SMTP settings matching 2.15
     ```
2. **Verify Pods Health**:
   * Ensure that the operator reconciles the APIManager and all pods (system-app, system-sidekiq, backend-listener, apicast-production, apicast-staging) transition to `Running`.

#### 4.2.5 Phase 5 — Cutover, Integration & Verification
1. **Update Single Sign-On OIDC Trust**:
   * Navigate to the 3scale Admin Portal under the new cloud domain.
   * Go to **Integration > SSO** and reconfigure the OpenID Connect integration to trust the newly migrated Keycloak cluster running on AWS EC2 (fetching JWKS keys from `https://sso.payu.ocp.fajjjar.my.id/auth/...`).
2. **Inbound Traffic Switch**:
   * Switch the official API domain pointer in Route 53 to point to the new `apicast-production` load balancer.
3. **Internal & Outbound Verification**:
   * Verify Service-to-Service traffic loops through the cloud APIcast gateways.
   * Ensure outbound traffic to TokoBapak and Nobar successfully exits through the cloud NAT gateway IPs (which are whitelisted on partner firewalls).
4. **Validation Requests**:
   * Send test requests with API Keys:
     ```bash
     curl -k -I https://api.payu.ocp.fajjjar.my.id/v1/accounts -H "user-key: <API_KEY>"
     ```
   * Send test requests with JWT tokens to verify that APIcast contacts RH-SSO, validates the signature, and accepts the authentication.
