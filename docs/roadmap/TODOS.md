...
| NOTIF-001 | P3 | [notification] Fixed 2026-07-31: env live `QUARKUS_OIDC_TENANT_ENABLED=false` (hack runtime, tak ada di repo) mematikan tenant OIDC Quarkus → `@Authenticated` selalu 401 "Not Authenticated". Hapus env → notification-health ALL 3 PASSED (T1 200, T2 200, T3 401). Broker AMQ fix (L-154): policy readonly-root-fs exclusion + selector `payu-broker-hdls-svc` ke label operator. | ✅ Done |
