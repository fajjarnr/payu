...
| GRPC-001 | 🔴 | account | Proto `AccountService` defined tapi **0 implementasi server** — **CLOSED 2026-08-12**: server `AccountGrpcService` live (9090, 6 test) DAN pemanggil bermigrasi: transaction `AccountServiceAdapter` → gRPC `GetAccountsByUser` (CB/Retry + fail-safe empty list), lending `AccountGrpcClient` untuk account-ids (profile lookup `getUserById` tetap REST — belum ada RPC gRPC). Deployed 1.10.74 | AccountGrpcService; AccountServiceAdapter; AccountGrpcClient |
