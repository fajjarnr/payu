...
| 37 | **AUDIT-037** ✅ CLOSED | ~~P2~~ | Spring Boot | **CLOSED 2026-07-02**: Verified that `IdempotencyFilter.java` in `gateway-service` acts as the central firewall, enforcing presence of standard/legacy idempotency headers on write requests targeting all 21 core financial routes (disbursements, transfers, payments, FX, lending, checkout, partners, etc.). Verified by `IdempotencyFilterEnforcedTest`. | `IdempotencyFilter.java:152-163` verified. |
...
| 40 | **AUDIT-040** ✅ CLOSED | ~~P3~~ | DevEx | **CLOSED 2026-07-02**: Configured Renovate Bot in `.github/renovate.json` with recommended presets, security:automerge for patch updates, and grouped PinActions to automate dependency updates. | `.github/renovate.json` created. |
| 41 | **AUDIT-041** ✅ CLOSED | ~~P3~~ | Security | **CLOSED 2026-07-02**: Documented as by-design. A deterministic key is required for local multi-pod dev environments to share encrypted data (BUG-SHARED-002 fix). Fails-fast in production/container profiles if no secure password is set, preventing default key usage in prod. | `SecurityAutoConfiguration.java:158` (by design) |
...
| 46 | **AUDIT-046** ✅ CLOSED | ~~P3~~ | Next.js | **CLOSED 2026-07-02**: Verified that `NEXT_SERVER_ACTIONS_ENCRYPTION_KEY` is already mapped from the `session-secrets` Kubernetes Secret in the base deployment manifest ([deployment.yaml](file:///home/ubuntu/payu/infrastructure/workloads/base/web-app/deployment.yaml#L94-L99)). | `NEXT_SERVER_ACTIONS_ENCRYPTION_KEY` mapped in deployment.yaml. |
| 47 | **AUDIT-047** ✅ CLOSED | ~~P3~~ | Architecture | **CLOSED 2026-07-02**: Verified that the regex `^payu\.[a-z][a-z0-9-]*\.[a-z][a-z0-9-]*\.v[0-9]+(?:\.dlq)?$` is correct and properly supports multi-segment domains using standard event-type names (e.g. `payu.accounts.savings-goal-created.v1`). Segment hierarchy follows AGENTS.md. | Regex audited; matches design spec. |
...
| 58 | **AUDIT-058** ✅ CLOSED | ~~P3~~ | Testing | **CLOSED 2026-07-02**: Parameterized hardcoded passwords in test configurations (`auth-service`, `dispute-service`) to use safe environment variables with default fallback placeholders (e.g., `${TEST_DATABASE_PASSWORD:payu}`). | `auth-service` and `dispute-service` test configs updated. |
...
| 63 | **AUDIT-063** ✅ CLOSED | ~~P3~~ | Next.js | **CLOSED 2026-07-02**: Verified that `serverActions.allowedOrigins` is indeed configured properly in `next.config.ts` (lines 17-29). Combined with the closure of AUDIT-046, Next.js Server Actions security is fully resolved. | `next.config.ts` verified. |
...
| 74 | **AUDIT-074** ✅ CLOSED | ~~P3~~ | Resilience | **CLOSED 2026-07-02**: Documented as by-design. Resilience4j library API (`failureRateThreshold` and `slowCallRateThreshold`) explicitly requires `float` types. PayU property declarations align with library signatures. | No code changes required; matches library design. |
...
| P3 nice-to-have | 0 (AUDIT-074 ✅, 075 ✅, 077 ✅ CLOSED) |
