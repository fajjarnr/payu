...
| KAFKA-QUORUM-001 | P2 | **FIXED 2026-09-12 (akar: NetworkPolicy, bukan quorum)** — `allow-kafka-platform` (egress) blackhole DNS khusus pod label-strimzi (bukti diferensial: pod-IP OK vs ClusterIP FAIL, pod fresh tanpa label OK, dev TANPA policy ini sehat) → broker tak resolve voter → `FATAL caught up` 25 jam+. Fix: hapus policy di sit/uat/preprod (+backup `/tmp/allow-kafka-*-backup.yaml`) → SIT 1 broker + 3 controller (scale dari 1, samakan dev) + UAT/preprod 1+1 SEMUA 1/1, CR Ready. Wipe PVC SIT hanya simtomatik (volume fresh ikut fatal sebelum DNS pulih). **RISIKO KEMBALI**: policy bawa anotasi ArgoCD sync-wave (sumber infra-repo?) → bisa muncul lagi saat sync infra; bila Kafka down lagi, cek policy ini dulu | `payu-kafka-broker-0` 228 restart; `getent`/`nslookup` hang di semua pod kafka, gateway 1 node OK; hapus policy → DNS <20s → broker join | CLOSED (SIT/UAT/preprod hijau); tindak lanjut = hapus permanen di sumber infra-repo + tambah guard DNS-probe kafka ke verify-overlay-drift |
...
| KAFKA-UAT-001 | P1 | **Broker UAT CrashLoop: DNS gagal khusus dari pod Strimzi** — broker-0 `UnknownHostException payu-kafka-controller-1…` 40×; controller-1 sehat (leader, ~300 events/min). Terbukti BUKAN: policy (tak ada egress-deny, rule izinkan), DNS server (pod lain se-node resolve instan), TCP (pod-IP reachable), sertifikat (valid s/d 2027), nsswitch/resolv (identik). `getent` dari pod kafka timeout 25-40s, pod lain instan — anomaly CNI/DNS per-pod. | log JVM `UnknownHostException` + `getent` per-pod matrix + cert valid + endpoint ada | OPEN — butuh akses node (oc debug/OVN trace) atau recreate StrimziPodSet; di luar remit app-loop. Dampak: outbox tak drain, wallet tak terprovision, tahap wallet k6 merah. Pola sama dengan Kafka SIT crashloop (parked) |
