...
| **Backlog Aktif** | **8 OPEN** — Grill P1 **7/7 CLOSED** 1.18.60-1.18.62 ✅ + **P3 Extended 3/3 CLOSED 1.18.63-1.18.64** ✅ + **Hotfix 1.18.65 Insecure crypto.randomUUID Fallback** ✅ + **UI Responsive 1.18.66 Web-App Mobile-Friendly** ✅ + **BFF/OpenAPI 1.18.67 Beneficiary A3** ✅ + **Wallet Idempotency 1.18.68 Pocket** ✅ + **Lint Polish 1.18.69 0 errors** ✅ + **Docs README 1.18.70 Next16** ✅ + **Lint 0 warnings 1.18.71** ✅ + **Wallet Pocket Version 1.18.72** ✅ + **Audit No-Warn 1.18.73** ✅ + **Audit API Consistency 1.18.74** ✅ + **Diet DRY/Bundle/Image 1.18.75 d6a37a2b2** ✅ — **NEW 8 OPEN 1.18.76 per-service re-audit (PER-SVC-001..008)** |
| **Last Updated** | 2026-08-30 — **1.18.75 Diet DRY/Bundle/Image d6a37a2b2** + **1.18.76 Per-Service Re-Audit 8 OPEN (PER-SVC-001..008: LENDING-MONEY, FX-MONEY, DISPUTE, TRANSACTION/ACCOUNT/LENDING/INVEST/LOP idempotency, SUPPORT outbox, GATEWAY CORS, PII, HEALTHCHECK)** |
...
### Audit 2026-08-30 — Per-Service Backend + Web-App (1 by 1) — 8 OPEN (codegraph 4 scout)

> Run: `codegraph_explore 28 service + 5 simulator + web-app 40 routes` + `grep` 8-point (Money 19,4 HALF_EVEN, X-Idempotency-Key, PII mask, outbox, hex, test, container, TODO). Web-App 8/8 PASS (branded Money, 100% financial POST, httpOnly Strict, native calendar/carousel, 34 deps). Simulators/python Money Decimal string PASS. Core/support: 3 🔴 FAIL + 4 🟡 WARN.

| Key | Pri | Temuan | Bukti | Sisa |
|---|---|---|---|---|
| PER-SVC-001 | P1 | **LENDING-MONEY scale 2→4** — `LendingApplicationService:281 principal.divide(months,2,HALF_EVEN)` + `LoanPreApprovalService:139 divide(2)` + `LendingDmnService:73 setScale(2)` vs `AGENTS.md 19,4` + `InstallmentService 4` | `backend/lending-service/src/main/java/id/payu/lending/application/service/LendingApplicationService.java:281` `LoanPreApprovalService.java:139` | OPEN |
| PER-SVC-002 | P1 | **FX-MONEY Settlement 2→4 + idempotency** — `SettlementFxRate:86 convert setScale(2)` vs `FxRate:57 4` + `FxController` no `X-Idempotency-Key` | `backend/fx-service/src/main/java/id/payu/fx/domain/model/SettlementFxRate.java:86` `FxController` no `@Idempotent` | OPEN |
| PER-SVC-003 | P1 | **DISPUTE-REFUND idempotency no column** — `@Idempotent required=true` tapi `RefundEntity` tanpa `idempotencyKey`/`UNIQUE`, `RefundService 37` duplicate on replay | `backend/dispute-service/src/main/java/id/payu/dispute/adapter/persistence/entity/RefundEntity.java` `RefundJpaRepository` | OPEN |
| PER-SVC-004 | P1 | **TRANSACTION-QRIS required false→true + ACCOUNT-BUDGET + LENDING-PAYLATER + INVEST-ACCOUNTS + LOP 2 POST** — 6 endpoint financial tanpa `required=true` | `TransactionController:473 qris required=false` `BudgetController:50` `LendingController:292 paylater required=false` `InvestmentController:64` `LoanOriginationController:33` | OPEN |
| PER-SVC-005 | P2 | **SUPPORT-OUTBOX log-only** — `ticket-created` via `Logger.info` tanpa `outbox-starter`, no DLQ, fail-open | `backend/support-service/src/main/java/id/payu/support/adapter/web/SupportController.java:339` `ponytail: outbox → log` | OPEN |
| PER-SVC-006 | P2 | **GATEWAY-CORS legacy Idempotency-Key** — `IdempotencyFilter` terima `X-Idempotency-Key`+`Idempotency-Key` tapi `application.yaml:169 allowed-headers` hanya `X-` | `backend/gateway-service/src/main/resources/application.yaml:169` `IdempotencyFilter:53` | OPEN |
| PER-SVC-007 | P3 | **PII amount log** — `lending/loan-origination` `log.info userId, amount` tanpa mask, `LendingController:207` `DisbursementService:40` | `backend/lending-service/src/main/java/id/payu/lending/adapter/web/LendingController.java:207` `backend/loan-origination-process/src/main/java/id/payu/loanorigination/service/DisbursementService.java:40` | OPEN |
| PER-SVC-008 | P3 | **HEALTHCHECK missing** — `lending-service` `investment-service` `NO HEALTHCHECK` vs others `HEALTHCHECK /actuator/health/liveness` | `backend/lending-service/Containerfile` `backend/investment-service/Containerfile` | OPEN |
