...
| **Last Updated** | 2026-08-26 — audit login/register payu-dev: 6 lapis defect SSO + CSP hydration + RLS GUC diperbaiki, E2E hijau (`CHANGELOG.md` 1.18.47); sisa OPEN: RLS rollout, issuer audit env lain, DPoP, rate-limit keying; **grill pipeline-perf**: rencana CICD-PERF-001..004 + CICD-DRIFT-001..003 + CICD-CLEANUP-001 masuk backlog, BELUM dieksekusi. |
...
### Audit 2026-08-26 — Pipeline Performance & Drift Dokumen↔Pipeline (grill; rencana disetujui, BELUM dieksekusi)
...
| Key | Pri | Temuan | Bukti | Sisa |
|:---|:---:|:---|:---|:---|
| CICD-PERF-001 | P2 | **Cache dependency mati dua lapis** — `maven-java21-task.yaml:62` override `-Dmaven.repo.local=$(workspaces.source.path)/.m2` mengabaikan mount; workspace `m2-cache` pakai `volumeClaimTemplate` → PVC baru kosong tiap run → full download tiap build | Maven 5m48s/build; Context7 tektoncd/pipeline workspaces.md: shared data wajib `persistentVolumeClaim.claimName` | PVC `m2-cache-shared` RWO gp3→RWX efs-csi; repo.local → `$(workspaces.m2-cache.path)`; DB cache grype/trivy → PVC sama; Task apply cluster-wide dulu, binding `claimName` pilot transaction/wallet/va-simulator |
| CICD-PERF-002 | P2 | **k6 smoke dipaksa jadi load test** — default task `--vus 20 --duration 2m` menimpa profil script (CLI flag = precedence tertinggi, Context7 grafana_k6); script `local-smoke.js` hanya 20s | k6 TaskRun 4m9s di setiap build dev (`k6-task.yaml:17-21,52-61`) | `k6-task.yaml`: flag kondisional bila param kosong; readiness-wait ≤60s dipertahankan; berlaku cluster-wide saat apply |
| CICD-PERF-003 | P2 | **Rantai DAG strictly sequential untuk task independen** — gitleaks/trufflehog/semgrep read-only; syft/grype/trivy/rhacs scan image identik (grype scan image langsung, bukan SBOM syft); zap & k6 hanya menembak HTTP | Rantai `runAfter` di 32 file pipeline; syft 20s/grype 137s/trivy 30s/rhacs 15s berurutan | Hapus edge `runAfter` tanpa aliran data; **invariant: cosign menunggu SEMUA scanner selesai sebelum sign→deploy (gate-before-deploy)**; cosign hanya butuh sbom.json dari syft; zap∥k6 setelah sync-wait; pilot 3 service → ukur ≥3 run vs p50≤10m/p95≤12m/0 gate skip → roll 28 sisa |
...
| CICD-DRIFT-001 | P1 | **SpotBugs+FindSecBugs tidak terpasang** — DEVSECOPS §4.1.3 "wajib untuk service Java", deskripsi YAML mengklaim ada, tapi tidak ada satu pun pom yang memuatnya | `grep -r spotbugs backend --include=pom.xml` = 0 file | Perbaikan TERPISAH dari perf changeset (menambah waktu build); wiring via parent-pom profile + keputusan blocking vs report-only |
| CICD-DRIFT-002 | P1 | **CI tidak mengenakan unit test** — `payu-test-pipeline` (unit/architecture/python/integration/pact) ada dan sehat tapi 0 eksekusi; semua build `-DskipTests`; verifikasi `mvn test` hanya manual saat rilis — konflik dengan TDD AGENTS.md | `oc get pipelinerun -n payu-cicd` match "test" = 0; `test-pipeline.yaml` tak direferensikan PipelineRun mana pun | Desain tersendiri (serius untuk platform pembayaran): wire in-path cepat vs nightly — ikuti tiering ADR-0071; jangan di-bundel dengan perf agar baseline before/after bersih |
| CICD-DRIFT-003 | P3 | **Nuclei + Dredd diklaim Stage 3** (§4.3.1/§6.2 "Wajib") tapi tidak ada task-nya di pipeline mana pun | grep cocok hanya string deskripsi `payu-service-pipeline.yaml:12-13` | Putuskan saat desain DRIFT-001/002: buat task Nuclei/Dredd atau koreksi PRD ke status opsional |
| CICD-CLEANUP-001 | P3 | **±150 PVC yatim di `payu-cicd`** — `volumeClaimTemplate` (source 5Gi + m2 1Gi per run) tidak pernah dibersihkan | `oc get pvc -n payu-cicd` ~150 Bound, umur hingga 40h+ | Hapus PVC yatim + evaluasi TTL/cleanup CronJob; aman dihapus setelah PipelineRun induknya selesai |
---
