...
| **READY-039** | **Architecture** | **Resolve 7 pre-existing ArchUnit violations** (account, product-catalog, transaction, investment, support, cms, integration). Architecture refactor leftovers (e.g., `..adapter..` depending on `..dto..` package). P2. Each violation is a 1-3 file fix to either relocate DTOs to adapter layer OR update ArchUnit rule to allow the dependency. Estimated: 0.5 day per service. | 0% | 100% |
...
| **READY-041** | **Security** | **partner-service Spring Security config audit** — `SecurityConfig` bean creation fails with `UnsatisfiedDependencyException` on `securityFilterChain`. SB 4.1.0 / Spring Security 7.0 API change. Estimated: 2-4h to refactor SecurityConfig. | 0% | 100% |
