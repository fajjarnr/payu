...
| **Last Updated** | 2026-08-03 (MVP-003 VA settlement, PROD-019 refund authorization, PROD-020 dispute IDOR, PROD-001 refund source-of-truth leg, and PROD-002 FX stub isolation updated; PROD-001 duplicate/reversal execution remains with PROD-021; PROD-002 real provider/freshness/audit remains open; OPS-2026-08-01-03/04 resolved — root cause kyverno default-deny + OVN egress, fix L-188; kraken gate pending capacity; DR restore verified) |
...
|---|---|---|---|---|
| PROD-001 | P0 | Dispute/refund | Source-of-truth leg fixed: full/partial refund now reads validated amount/currency through a read-only transaction port; duplicate-request and reversal-ledger execution remain tracked with PROD-021. | Integrasikan port read-only ke transaksi asli, ambil amount/currency tervalidasi, lalu uji full/partial refund, duplicate request, dan reversal ledger. |
| PROD-002 | P0 | FX | Stub sekarang hanya aktif pada profile `local` dan unknown pair tidak lagi mendapat fallback `1`; profile non-local memakai adapter fail-closed (`isAvailable() == false`) saat `realFxRateProvider` belum dikonfigurasi. Real provider, freshness/source check, dan pair/rate audit masih belum ada. | Stub hanya boleh profile local (test memakai mock provider); non-local wajib real provider, freshness/source check, fail-closed, dan pair/rate audit test. |
