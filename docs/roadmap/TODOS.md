...
| COMPLIANCE-HARDEN-001 | compliance-service / ADR-0063 | **AML/CFT + PCI-DSS Req10 audit trail**: `AuditReport` + `DataAccessAudit` append-only `REVOKE UPDATE,DELETE` `version`, structured JSON `traceId/businessId/tenant`, WORM 1y (3m immediate) +7y cold, daily review, dual approval >50M IDR `maker≠checker`. Ref ADR-0063 + ADR-0030 Velocity + ADR-0032 WAF/SIEM | `REVOKE` check + disputed-payout `traceId` e2e reconstructable |
| GATEWAY-HARDEN-001 | gateway-service / ADR-0064 | **3scale APIcast edge + rate limiting**: `APIcast` hybrid async (manager vs gateway), `application plan` per Account/Application + `edge limiting` `leaky_bucket/fixed_window/connection_limiters=100` before APIcast via external Redis (Hot Rod), per-user `{{jwt.claim.sub}}` liquid template fairness, `IP check` + `maintenance mode`. Ref ADR-0064 + Red Hat 3scale | `edge_limited_total` metric + 100-concurrent protected vs 1000-apps burst |
| PORTAL-HARDEN-001 | api-portal-service / ADR-0065 | **OpenAPI aggregation DX**: `GroupedOpenApi public/admin` SpringDoc, `ApiPortalService.java:27` `ConcurrentHashMap` TTL `PT5M` partial-failure `failedServices`, 3scale ActiveDocs OAS 3.0 `x-data-threescale-name=user_keys`, contract test `spring-cloud-contract` Pact CI. Ref ADR-0065 + ADR-0056 Simulator Fidelity | `refreshCache` partial 1/N down still 200 + ActiveDocs try-it green |
