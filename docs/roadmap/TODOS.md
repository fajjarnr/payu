...
|:---|:---|
| **Open P0s** | 0 |
| **Open P1s** | 26 (18 closed: READY-001, 002, 010, 011, 012, 013, 015, 016, 017, 018, 019, 020, 021, 022, 023, 024, 025, 026, 027, 028, 029, 030, 031, 032) + READY-033 + READY-035 (test code partial) |
| **Open P2s** | 12 |
| **Open P3s** | 4 (READY-060, 061, 062, 063) |
| **Production Score** | ~75% compile / ~25% runtime (compile-clean across all 22 services + 14 shared starters, but `mvn -T 1C test` shows 20/41 modules still skipped due to upstream test failures in saga/outbox starters from Jackson 3 ABI break) |
| **Last Audit** | June 15, 2026 — READY-031 + READY-032 + READY-033 + READY-035 closed. UPGRADE-013 (Quarkus 3.36.2) done. READY-034 partial (main code + poms done, test code migrated but blocked by Jackson 3). Hypersistence JsonType → Hibernate 7 native JSON for 2 starters. NEW ticket: Jackson 3 strategy decision (force 2 vs full 3 migration). |
...
| **READY-036** | **Platform decision** | **Jackson 2 vs Jackson 3 strategy for SB 4.1.0** — runtime blocker for saga/outbox starters + 20 cascaded services. SB 4.1.0 defaults to Jackson 3 (`tools.jackson.databind.*`) but `JsonMapper.Builder.<clinit>` requires `com.fasterxml.jackson.annotation.JsonSerializeAs` (REMOVED in Jackson 2.18). Result: `NoClassDefFoundError` at first Jackson 3 init. `spring-boot-autoconfigure-classic` alone is INSUFFICIENT (Jackson 3 jar still on classpath). **3 options**: (A) force Jackson 2 platform-wide by excluding Jackson 3 modules — 1-2 days, deprecated path; (B) full Jackson 3 migration to `tools.jackson.*` — 1-2 weeks, long-term correct; (C) wait for SB 4.x patch — unclear. **Stakeholder decision needed** before any further test runtime work. Discovered 2026-06-15 during READY-034 subagent `mvn -T 1C test` run. See [LESSONS.md L-041](docs/guides/LESSONS.md). | 0% | 100% |
...
_Last Updated: June 15, 2026 — READY-031 + READY-032 + READY-033 + READY-035 closed. READY-034 partial (main code + poms done, test framework migrated, test runtime blocked by Jackson 3). UPGRADE-013 (Quarkus 3.36.2) done. Hypersistence JsonType → Hibernate 7 native JSON for 2 starters. NEW P1 ticket READY-036 (Jackson 3 strategy) opened. Production readiness: 75% compile / ~25% runtime._
