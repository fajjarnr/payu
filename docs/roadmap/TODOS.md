...
| OPS-2026-08-01-04 | P2 | Observability | Log delivery: vector collector → `loki-gateway-http.openshift-logging.svc` DNS lookup gagal dari pod di openshift-logging (`Name or service not known`/hang) walau egress DNS (namespaceSelector + ipBlock 172.30.0.10:53) sudah diizinkan. CLF Ready; log audit belum sampai Loki. | 🔄 Investigate |
| OPS-2026-08-01-05 | P2 | Chaos | Kraken/preprod gate runtime: cerberus FIX (kubeconfig_path — L-185, "client set"; SCC anyuid + FS writable). Sisa: krkn container rootfs di-mount `ro` oleh CRI-O walau `readOnlyRootFilesystem: false` (mount evidence: `overlay on / type overlay (ro,...)`) → `kraken.report` Read-only. Teori: image-specific (krkn `VOLUME`/runtime), bukan Kyverno/SCC (cerberus image di pod sama jalan tulis). Opsi: emptyDir utk `/home/krkn/kraken` + entrypoint override, atau ganti image krkn. | 🔒 Investigate |
| INFRA-026 | P1 | Secrets | **2026-08-01**: DR drill manifest `dr-drill.yaml` ready (scratch vault awskms seal + egress NP + command override — L-187); restore verification pending (exec ke node baru no route; snapshot 20260801T005422Z.snap di S3). | 🟢 Live — restore drill in progress |
