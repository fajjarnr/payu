...
| ARCH-GLOBAL-005 | platform | Implementasi Database High Availability, Continuous PITR & DR Restore Drill sesuai [ADR-0031](../adr/0031-database-resilience-pitr-and-disaster-recovery.md) & [ADR-0006](../adr/0006-postgresql-primary-database.md): (1) Konfigurasi `barmanObjectStore` S3 continuous WAL streaming (`archive_timeout=60s`) + CSI VolumeSnapshot harian di `ha-patch.yaml` (CNPG 1.30+), (2) Modernisasi runbook `docs/operations/DISASTER_RECOVERY.md` (hapus legacy Crunchy PGO / patronictl dan ganti dengan prosedur native CNPG & AWS RDS Multi-AZ), (3) Otomasi skrip DR drill di `scripts/backup-dr/` (`dr-cnpg-failover-drill.sh`, `dr-cnpg-pitr-restore.sh`, `dr-verify-ledger-integrity.sh`), (4) Uji failover otomatis RTO < 5m dan PITR data integrity check (RPO=0, double-entry ledger balance debit==credit). | Automated PITR restore drill verified di testing cluster + runbook CNPG diperbarui + failover RTO<15s terbukti |
| ARCH-GLOBAL-006 | security | Implementasi Perimeter Security (Tiered WAF & Centralized SIEM) sesuai [ADR-0032](../adr/0032-perimeter-security-waf-coraza-and-siem-wazuh.md): (1) Deployment Coraza WAF (OWASP CRS v4.x, PL1/PL2, SNAP-BI exclusions `X-SIGNATURE`/`X-CLIENT-KEY`, max body 128KiB, Anomaly Threshold 5) pada Ingress OpenShift, (2) Wazuh SIEM cluster (`wazuh-manager-scc` with `SYS_CHROOT`, `anyuid` for Indexer/Dashboard, `privileged` for Agent DaemonSet), (3) OpenShift `ClusterLogForwarder` output sink Syslog RFC 5424 (`tcp://wazuh-manager.wazuh.svc.cluster.local:514`), (4) Multi-source correlation: WAF alerts + Falco syscalls + Keycloak auth + Vault audit ke Wazuh Indexer + Sev≥10 alerting. | Wazuh dashboard live + CLF audit log arriving + Coraza blocking malicious SQLi/XSS test payload |
| ARCH-GLOBAL-007 | data-security | Implementasi Database Row-Level Security (PostgreSQL RLS) & Multi-Tenant Isolation sesuai [ADR-0033](../adr/0033-database-row-level-security-and-multi-tenant-isolation-standard.md): (1) `TenantAwareTransactionSynchronization` & `TenantAwareHikariDataSource` di `datasource-starter` (`SET LOCAL app.tenant_id = :id` saat transaksi & `RESET` pada connection close), (2) Pemisahan DB Role `payu_migrator` (Owner, `BYPASSRLS`) vs `payu_app` (`NOBYPASSRLS`), (3) Flyway DDL `FORCE ROW LEVEL SECURITY` + policy fail-closed `AS RESTRICTIVE` pada seluruh 27 tabel tenant di 6 microservices, (4) Keycloak JWT claim `partner_id`/`tenant_id` validation di `TenantFilter` + gateway header sanitization, (5) Enable kembali integration test `BlindIndexAndTenantIsolationIntegrationTest` di `account-service` + cross-tenant matrix test 100% green. | `BlindIndexAndTenantIsolationIntegrationTest` pass + DB non-admin query terisolasi mutlak (0-row saat mismatch) |
| ARCH-GLOBAL-008 | observability | Implementasi End-to-End Observability, Distributed Tracing (W3C/OTel/Tempo), dan Multi-Window Multi-Burn-Rate SLI/SLO sesuai [ADR-0034](../adr/0034-end-to-end-observability-slo-sli-and-distributed-tracing-standard.md): (1) Multi-Window Multi-Burn-Rate PromQL alerts di `slo-alerts.yaml` (1h/14.4x & 6h/6x PagerDuty page, 24h/3x & 3d/1x Slack ticket), (2) Propagasi W3C `traceparent` di HTTP headers, Kafka CloudEvents (`ce_traceparent`), dan PostgreSQL SQLCommenter, (3) OTel Collector pipeline & tail-sampling (100% financial/errors, 5% reads, 0% health probes) + TempoStack S3 sink, (4) PII masking filter (zero PAN/PIN/CVV/NIK di logs & traces), (5) Multi-tenant partner dashboard (SNAP-BI TPS, latency, error code breakdown RFC 9457) di Grafana. | Multi-burn-rate alerts active + W3C trace linked end-to-end (APIcast->Quarkus->Kafka->SpringBoot->PG) + Tempo traces arriving |
...
|:---|:---:|:---|:---|
| PARTNER-PROD-001 | P0 | 🟢 Public edge APIcast LIVE (sandbox): E2E luar cluster 200, quota 429, failover OK, bypass route dihapus ([ADR-0025](../adr/0025-snap-bi-and-partner-gateway-security-standard.md)) | WAF Coraza (DEPLOY-006 / [ADR-0032](../adr/0032-perimeter-security-waf-coraza-and-siem-wazuh.md)), mTLS APIcast→gateway, rate-limit per-IP, runbook restart apicast |
...
| PARTNER-PROD-006 | P0 | 🟢 Tenant isolation LIVE (ownership semua resource, isolation matrix 295/295, audit — [ADR-0033](../adr/0033-database-row-level-security-and-multi-tenant-isolation-standard.md)) | PostgreSQL RLS GUC integration, partner-scoped Keycloak roles, audit list query |
| PARTNER-PROD-007 | P1 | ⏸️ Belum | HPA≥3, PDB minAvailable 2, topology spread, bounded timeout |
| PARTNER-PROD-008 | P0 | ⏸️ Belum | PG HA+PITR via CNPG Barman Cloud in-cluster & AWS RDS Multi-AZ target ([ADR-0031](../adr/0031-database-resilience-pitr-and-disaster-recovery.md), [ADR-0006](../adr/0006-postgresql-primary-database.md)), restore drill, RPO=0 / RTO<5m, retention/archive |
| PARTNER-PROD-009 | P1 | ⏸️ Belum | SLI/SLO, dashboard+alert, traces end-to-end ([ADR-0034](../adr/0034-end-to-end-observability-slo-sli-and-distributed-tracing-standard.md)) |
...
|:---|:---:|:---|:---|
| DEPLOY-006 | P1 | Security | Coraza WAF (INFRA-015) + Wazuh SIEM (INFRA-011) + sisa CIS `audit-log-forwarding-enabled` sink sesuai [ADR-0032](../adr/0032-perimeter-security-waf-coraza-and-siem-wazuh.md) |
