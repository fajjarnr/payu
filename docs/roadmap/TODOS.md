...
| MVP-004 | P1 | Idempotency boundary untuk SNAP payment/refund dan disbursement callback kini wajib `X-Idempotency-Key`; natural-key/unique index V15/V16 dan HMAC callback sudah tersedia. Sisa: serialisasi refund cumulative-sum agar concurrent refund tidak lolos bersamaan. | 🟡 Partial — endpoint guards implemented locally; refund concurrency lock + live E2E pending |
...
### 🟡 MVP-004: Idempotency boundary — endpoint guard selesai, refund race masih terbuka
...
* **Remaining**: `createRefund` masih menghitung `sumRefundedAmount` secara non-atomic. Tambahkan row lock pada payment parent atau constraint/ledger reservation yang serializable sebelum mengklaim MVP-004 selesai penuh.
* **Verification**: controller contract tests SNAP 1/1, disbursement 1/1, callback signature 10/10, `partner-service` 238/238, dan `transaction-service` 132/132 lulus pada 2026-08-03.
