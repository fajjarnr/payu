...
| FX-002 | P2 | [fx] `POST /api/v1/fx/conversions` masih 503: wallet debit via gRPC `ConnectTimeout` — `wallet-service` tidak punya gRPC server (no grpc config/dependency) dan Service tak expose port gRPC; `WALLET_GRPC_ADDRESS` tak terdefinisi di fx. Gap integrasi pre-existing, butuh grpc-starter + handler di wallet-service. | 🔄 Planned |
