...
| RELAY-010 | P2 | **Seed job dev permanen** — wallet/account/seed + realm users hanya manual; dev baru = journey mati lagi. | Temuan sesi ini | CLOSED — Flyway permanen: wallet `V123` (5 wallet, ON CONFLICT, tanpa ledger) + account `V113` (users/profiles/accounts link KC sub, ON CONFLICT); live `payu-dev` teraplikasi sbg role `payu` (wallets 5 exact, users 3 link, re-run 0/0/0) + runbook `docs/operations/runbooks/dev-seed.md` (realm re-sync flow) → `CHANGELOG.md` `1.18.100` |
...
| FE-AUDIT-006 | P2 | **Analytics backend nol padahal transaksi ada** — akar: outbox tak pernah drain (bean mati) + consumer bind `None` + event tanpa identitas. CLOSED sisi pipa (drain 68+40, replay, `user_id` di event baru). Sisa: event lama keyed account-id; wallet event tanpa userId. | proof relay + API probe + Kafka lag 0 | PARTIAL — agregasi terisi mulai transfer baru |
