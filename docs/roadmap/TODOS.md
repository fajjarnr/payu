...
* **Gap 2 — callback eksternal**: `POST /api/v1/disbursements/callback` (DisbursementController:132) masih **tanpa `@Idempotent`**. VA callback sudah memakai HMAC + `@Idempotent` (MVP-003).
* **Fix**: `@Idempotent(required=true)` pada 3 endpoint tersisa + unique-constraint `partnerReferenceNo` di `snap_bi_payment`, + autentikasi/validasi HMAC callback.
