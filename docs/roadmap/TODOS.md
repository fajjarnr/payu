...
| MVP-004 | P1 | Idempotency boundary untuk SNAP payment/refund dan disbursement callback kini wajib `X-Idempotency-Key`; natural-key/unique index V15/V16, HMAC callback, dan parent-row lock untuk cumulative refund sudah tersedia. | 🟡 Partial — guards + refund lock implemented locally; live E2E pending |
| MVP-005 | P1 | Interbank transfer (BI-FAST/SKN/RTGS) **stuck PENDING selamanya**: `InitiateTransferCommandHandler.processBiFastTransfer` (baris 148-191) & `processInterBankTransfer` (:246-252) set status `PENDING` setelah `bifastServicePort.initiateTransfer`, tapi **TIDAK ADA mekanisme menyelesaikannya** → tidak pernah COMPLETED. Komentar kode "handled by downstream clearing system" merujuk sistem yang TIDAK ADA di repo: tidak ada Kafka consumer transfer-completed (hanya `BatchDisbursementService` consume `disbursement-batch`), tidak ada callback endpoint, dan `publishTransactionCompleted` hanya dipanggil internal transfer (:216) & QRIS (:113). Dampak: saldo sender di-reserve (terkunci) tapi tidak pernah di-commit/release/settle → dana membeku, tanpa reconciliation. Fix: lengkapi leg penyelesaian (consumer/callback BI-FAST/SKN/RTGS → commit + `publishTransactionCompleted`) ATAU release reservation dgn expiry (mirip escrow 5m) + reconciler. | 🔴 Unfinanced leg — stuck PENDING |
| MVP-006 | P1 | Webhook KELUAR **tidak idempotent**: `WebhookDispatcherService.dispatch()` (baris 101-116) selalu buat `WebhookDeliveryEntity` baru + `attemptDelivery` TANPA cek apakah `eventId` sudah terkirim (komentar field: "for idempotency on receiver side" → PayU andalkan integrator dedup). Tidak ada unique-constraint `(eventId, subscription)` di `webhook_deliveries` (cuma `uq_webhook_partner_url`). Karena outbox at-least-once (doc `OutboxPublisher`: crash-after-ack → duplikat) & `FinancialEventConsumer` tak mendedup sebelum `dispatch`, event yang dikonsumsi 2x → webhook 2x ke integrator + 2 row delivery. Fix: guard `existsByEventIdAndSubscription` + unique-constraint `(event_id, subscription_id)` di `webhook_deliveries`; return existing bila duplikat. Terkait MVP-004 (idempotency). | 🟢 Implemented — dedup guard `existsByEventIdAndSubscription_Id` + migrasi V16; build verified 2026-08-03 (partner-service `mvn test` SUCCESS, 235 tests 0 fail) |
...
| OPS-2026-08-01-06 | P1 | DB/App | Outbox lock leak di `payu_fx`: holder `PostgreSQL JDBC Driver` — awalnya `idle in transaction` 1h16m (update retry_count, RowExclusiveLock), setelah diterminasi muncul holder baru `SELECT ... FROM outbox_events ...` **active 1h+** memegang RowShareLock (SELECT FOR UPDATE dispatcher hang di sisi app) → semua SELECT/INSERT/ALTER outbox_events antri; `outbox-bootstrap` hang. Unblock live: `pg_terminate_backend` holder → job Complete 1/1. Manifest: `lock_timeout 15s` di job (fail fast, OPS tidak hang 4h lagi). Perlu fix app: JDBC outbox dispatcher connection hang (timeout/commit/reconnect). | 🔒 App bug — job completed; lock_timeout hardening applied |
...
### 🟡 MVP-004: Idempotency boundary — guard dan refund lock selesai lokal, live E2E pending
...
* **Remaining**: live wallet/OpenShift E2E setelah workload PayU tersedia.
* **Verification**: controller contract tests SNAP 1/1, disbursement 1/1, callback signature 10/10, `partner-service` 240/240, dan `transaction-service` 132/132 lulus pada 2026-08-03.

### 🔴 MVP-005: Interbank transfer (BI-FAST/SKN/RTGS) stuck PENDING — no settlement leg
* **Trace (2026-08-01)**: `InitiateTransferCommandHandler.processBiFastTransfer` (:148-191) → `bifastServicePort.initiateTransfer` → `setStatus(PENDING)` (:163); `processInterBankTransfer` (:246-252) → `setStatus(PENDING)` (:249), komentar "queued for downstream clearing... handled by the downstream clearing system". **Tidak ada sistem itu di repo.**
* **Tidak ada yang menyelesaikan**: (a) Kafka consumer `transaction.completed` untuk transfer — hanya `BatchDisbursementService.java:174` consume `disbursement-batch`; (b) callback endpoint transfer — hanya `BatchDisbursementController /{id}/complete` (batchnya, bukan transfer); (c) `publishTransactionCompleted` hanya dari internal (:216) & QRIS (:113) — tak pernah untuk BI-FAST/SKN/RTGS.
* **Dampak**: saldo sender di-reserve (Balanced held) namun tak pernah commit/release/settle → dana membeku PENDING tanpa expiry & tanpa reconciliation; `TransactionArchivalScheduler` cuma arsip, `PaymentExpiryScheduler` buat VA/QRIS, bukan transfer.
* **Fix**: (a) lengkapi leg settlement — consumer/callback BI-FAST/SKN/RTGS → `commit` + `publishTransactionCompleted`; ATAU (b) release reservation dgn expiry scoped (pola `EscrowService.processExpiredEscrows` 5m) + reconciler. Pola yang sama dgn MVP-001/003: leg eksternal tidak disambung ke mesin yang sudah ada.

### 🔴 MVP-006: Webhook keluar tidak idempotent — duplikat ke integrator
* **Trace (2026-08-01)**: `WebhookDispatcherService.dispatch(eventType, eventId, payload)` (baris 101-116) → utk tiap subscription: `new WebhookDeliveryEntity(...)` + `deliveryRepository.save` (baris 112-114) + `attemptDelivery` — **tanpa cek `eventId` sudah ada**.
* **Bukti no-dedup**: `@Table webhook_deliveries` tak punya unique-constraint `(event_id, subscription_id)` (cuma `uq_webhook_partner_url` di `V2__create_webhook_tables.sql:16`); komentar field `eventId` ("for idempotency on receiver side") = PayU serahkan dedup ke integrator.
* **Kombinasi risiko**: outbox at-least-once (`OutboxPublisher` doc: crash-after-ack → duplikat di retry, konsumen wajib idempotent) + `FinancialEventConsumer` tak mendedup sebelum `dispatch` + dispatcher tak dedup → event di-resume 2x = webhook 2x + 2 row `webhook_deliveries`. Integrator tetap bisa dedup via `X-PayU-Event-Id`, tapi PayU tidak menjamin at-most-once.
* **Fix**: `existsByEventIdAndSubscription` guard + unique-constraint `(event_id, subscription_id)`; duplikat → return existing (bukan kirim ulang). Beririsan dgn MVP-004.
...
|---|---|---|---|---|
| PROD-001 | P0 | Dispute/refund | `backend/dispute-service/.../RefundService.java:32-39,141-145` membuat full refund dengan amount `0.00` dan currency `IDR`; lookup transaksi masih placeholder. | Integrasikan port read-only ke transaksi asli, ambil amount/currency tervalidasi, lalu uji full/partial refund, duplicate request, dan reversal ledger. |
| PROD-002 | P0 | FX | `backend/fx-service/.../StubFxRateProviderAdapter.java:14-48` adalah `@Component` aktif dengan rate hardcoded dan `isAvailable() == true`; tidak ada `realFxRateProvider` di repo. | Stub hanya boleh profile local/test; non-local wajib real provider, freshness/source check, fail-closed, dan pair/rate audit test. |
| PROD-003 | P1 | Loan origination | `backend/loan-origination-process/.../LoanOriginationController.java:25-104` menyimpan workflow di `ConcurrentHashMap`; restart/replica dapat menghilangkan atau memecah state approval. | Persist state + approval transition dengan locking/versioning; approve wajib idempotent; identitas user berasal dari JWT, bukan `userId` request tanpa verifikasi role. |
| PROD-004 | P1 | Analytics money | `backend/analytics-service/src/app/database.py:42,65-67,98-99` memakai SQLAlchemy `Float` untuk amount/balance/change/total; service juga mengonversi melalui `float`. | Migrasikan field uang ke `NUMERIC(19,4)` dan Python `Decimal`; pertahankan float hanya untuk skor non-uang; tambahkan precision/invariant test. |
| PROD-005 | P1 | Analytics events | `backend/analytics-service/src/app/messaging/kafka_consumer.py:119-207` membuat UUID baru dan `float` setiap delivery; replay CloudEvent dapat menggandakan history/metric. | Simpan CloudEvent `id`/event identity asli, unique constraint/upsert atau dedup record, serta replay test untuk at-least-once delivery. |
| PROD-006 | P2 | Analytics schema/API | `database.py:157-179` menjalankan `create_all()` saat startup; `_get_income_by_source()` di `analytics_service.py:223-224` selalu mengembalikan `[]`. | Pakai migration versioned dan fail-fast bila schema tertinggal; implementasikan income source atau hapus endpoint/claim yang belum ada. |
| PROD-007 | P1 | Integration | `backend/integration-service/.../MessagePublisherAdapter.java:53-59` mengembalikan string sukses `gRPC response placeholder` tanpa call gRPC nyata. | Implementasikan client gRPC melalui port dengan auth, timeout, retry policy, dan error mapping; atau hapus capability dari API sampai tersedia. |
| PROD-008 | P1 | Gateway config/data | `InMemoryRatePlanRepository.java:16-33` dan `InMemoryPartnerRatePlanRepository.java:17-41` adalah bean `@ApplicationScoped` aktif dengan default partner/rate plan; test log juga menunjukkan in-memory transformation rules. | Rate plan, partner assignment, dan transformation rules harus durable/config-managed, tanpa default demo di prod; tambahkan locking, audit, cache invalidation, dan multi-replica test. |
| PROD-009 | P1 | Shared money API | `backend/shared/api-commons/.../Money.java:236-238,280-282` masih menerima `double` pada `multiply/divide`, bertentangan dengan aturan uang `BigDecimal`. | Hapus overload `double`, migrasikan caller ke `BigDecimal`/integer scalar, dan tambahkan ArchUnit/API guard agar overload kembali tidak muncul. |
| PROD-010 | P1 | Web auth/rendering | `frontend/web-app/src/proxy.ts:20-27,91+` menganggap cookie ada sebagai session; hasil build menunjukkan hampir semua protected pages SSG/static, bukan request-dynamic. | Validasi/decrypt session secara kriptografis di proxy atau enforce server-side auth sebelum sensitive RSC/HTML; uji cookie forged/expired/tampered, role denial, dan cache leakage. |
| PROD-011 | P1 | Web auth rate limit | `frontend/web-app/src/app/api/auth/{login,refresh}/route.ts` memakai `Map` process-local dan mempercayai `x-forwarded-for`; restart/replica atau header spoofing melewati limit. | Gunakan shared Hot Rod/edge limiter, trusted proxy identity, bounded counter/TTL, dan distributed abuse test. |
| PROD-012 | P1 | Web BFF resilience | BFF catch-all `frontend/web-app/src/app/api/v1/[...path]/route.ts:205-246` membaca body tanpa limit dan fetch upstream tanpa timeout; gateway URL punya fallback service default. | `GATEWAY_URL` wajib eksplisit di non-local; batasi body, gunakan AbortSignal timeout, batasi refresh/retry, dan uji upstream hang/oversize/401 loop. |
| PROD-013 | P1 | Web money | `frontend/web-app/src/types/index.ts:80-148` memakai `number` untuk balance/amount/fee; `src/lib/currency.ts` dan bills/pockets memakai `parseFloat`/`Math.round`. | Kontrak API gunakan decimal string/minor units; arithmetic mutation harus exact, formatting terpusat; uji `0.1`, `1.005`, dan amount besar IDR. |
| PROD-014 | P1 | Web probes/release | `frontend/web-app/src/app/api/health/route.ts:8-25` selalu 200, termasuk tanpa dependency check, dan hardcodes version `1.0.0`; base manifests memiliki version/image drift. | Pisahkan liveness dari readiness dependency-aware dengan timeout; version dari build/runtime metadata; samakan labels/service/image tag dan jadikan encryption secret wajib di prod. |
| PROD-015 | P0 | Web dependencies | `npm audit --omit=dev` gagal dengan 3 high severity, termasuk advisories pada Next.js, PostCSS, dan sharp; `npm audit fix --force` melampaui range dependency saat ini. | Upgrade terkontrol ke versi patched yang kompatibel, commit lockfile, lalu ulangi audit, test, build, dan security scan tanpa blind force-upgrade. |
| PROD-017 | P2 | Gateway observability | Gateway test log menunjukkan OTLP `Connection refused localhost:4317`; Quarkus mengabaikan `quarkus.smallrye-health.check-default-procedures` sebagai unknown key. | Perbaiki key/config profile dan collector endpoint; CI/deploy harus fail atau memberi signal jelas untuk config yang diabaikan dan telemetry yang wajib. |
| PROD-018 | P2 | Analytics CI | `python3 -m pytest -q` tidak dapat dijalankan di workspace (`No module named pytest`), walau `pyproject.toml` menetapkan coverage gate 80%. | Sediakan environment CI reproducible untuk analytics, jalankan pytest + coverage gate, dan publish hasilnya sebagai required check. |
...
| PROD-023 | P1 | Investment distributed consistency | `InvestmentApplicationService.java:126-190,220-270,292-352` mendebit wallet lalu menyimpan state lokal; kompensasi hanya immediate credit dan kegagalan kompensasi berakhir pada log “Manual intervention required”. | Persist saga/outbox state dan compensation reference, buat retry/reconciler durable, dan uji crash/timeout/duplicate di antara debit, save, dan event. |
| PROD-024 | P1 | Web investment truthfulness | `frontend/web-app/src/app/[locale]/investments/page.tsx:19-23,61-65,113-127,145-192` menampilkan return/risk/ROI/LPS/advice hardcoded; tombol buy/optimize/review tidak memiliki action, sementara data nyata yang diambil hanya account balance. | Bind angka/guarantee/advice ke backend yang authoritative atau tampilkan empty state yang jelas; wire action end-to-end atau hapus tombol yang belum berfungsi. |
| PROD-025 | P1 | Billing subscription authorization | `SubscriptionController.java:64-112,119-131,165-204` hanya memeriksa `isAuthenticated()` untuk partner/plan/subscription/charge route; menerima arbitrary `partnerId`/`accountId`, cancel tidak idempotent, dan ownership check hanya terlihat pada sebagian read route. | Enforce partner role/ownership dan account ownership di service boundary, tambahkan idempotency pada cancel, serta uji IDOR dan cross-partner access. |
| PROD-026 | P0 | Production PII/security defaults | `backend/account-service/.../application-container.yml:81-89` (pola sama di beberapa service) mematikan `masking-enabled` dan `audit-enabled`; `SecurityProperties.java:18,23-28` membolehkan default; `EncryptionService.java:62-72` memakai default PBKDF2 salt bila salt hilang. | Production profile wajib masking/audit aktif dan startup fail-fast bila key atau salt tidak ada; tambahkan config gate dan test bahwa identifier/amount tidak bocor ke log. |
| PROD-027 | P1 | Web auth state storage | `frontend/web-app/src/stores/authStore.ts:52-113` memakai Zustand `persist`/localStorage untuk `username`, `accountId`, `roles`, dan `isAuthenticated`; komentar mengklasifikasikannya sebagai non-sensitive, padahal sebagian adalah PII/stale auth state. | Simpan auth truth di server session/in-memory bootstrap, jangan persist PII atau `isAuthenticated`, migrasikan key lama, dan uji reload/logout/storage inspection. |
| PROD-028 | P1 | Web financial contract drift | `frontend/web-app/src/services/LendingService.ts:92-149` tidak memakai `getFinancialMutationHeaders()`; purchase/payment mengirim `null` body + query params, sedangkan `LendingController.java:270-303` membaca JSON body. `InvestmentService.ts:121-142` juga tidak mengirim header ke route yang mewajibkan `@Idempotent`. | Satu-kan typed request contract FE/BE, gunakan header pada seluruh required mutation, dan tambahkan contract/E2E test untuk body shape, 401, retry, dan duplicate replay. |
| PROD-029 | P1 | Royalty statement | `backend/wallet-service/.../SettlementService.java:314-333` membaca stakeholder tetapi tidak pernah menambahkan nilai ke `totalRoyalties`; statement selalu mencetak `0`. | Agregasikan settlement/journal persisted berdasarkan partner/account/periode, verifikasi rounding, dan tambahkan fixture statement non-zero serta reconciliation test. |
| PROD-030 | P1 | Promotion persistence | `backend/promotion-service/.../PromoCodePersistenceAdapter.java:16-34`, `PromoUsagePersistenceAdapter.java:17-45`, `CashbackRulePersistenceAdapter.java:15-36`, dan `CashbackRecordPersistenceAdapter.java:15-33` adalah bean aktif berbasis collection process-local, walau migration `V4__create_promo_cashback_tables.sql` sudah menyediakan tabel dan unique idempotency key. | Ganti dengan adapter durable/transactional atau batasi bean ke test profile; enforce unique/user-promo/idempotency constraints dan uji restart, multi-replica, serta concurrent redemption. |
| PROD-031 | P1 | Promo validation side effect | `PromoRedemptionController.java:110-129` menjalankan `applyPromo` untuk GET validation; `PromoCode.java:52-62,126-132` langsung menandai user/usage count sebagai used, lalu service menyimpan promo. Validasi dapat menghabiskan promo `ONCE_PER_USER`; endpoint apply juga menerima idempotency key optional. | Pisahkan pure validation dari apply/persist, wajibkan header idempotency pada apply, dan uji validate-then-apply, concurrent redemption, serta replay. |

### Follow-up audit — Graphify + CodeGraph

| ID | Pri | Area | Bukti | Minimum done |
|---|---|---|---|---|
| PROD-032 | P0 | Idempotency binding | `backend/shared/api-commons/.../IdempotencyInterceptor.java:122-156,230-289` membuat fingerprint dari URI/method/query/header tanpa body; authorization dan idempotency key dikeluarkan. `gateway-service/.../IdempotencyFilter.java:164-185,199-229` juga cache berdasarkan key saja dan fail-open saat cache gagal. Key yang sama dengan amount/account berbeda dapat replay response atau melewati validasi controller. | Hash canonical body dan bind ke principal/tenant/account; mismatch wajib `409`, cache failure pada financial path fail-closed, dan tambahkan negative test same-key/different-body serta cross-account. |
| PROD-033 | P0 | Wallet partial commit | `WalletGrpcService.java:227-241` commit debit sebelum credit remote; `SettlementService.java:288-301` commit batch sebelum loop credit; `SplitPaymentService.java:210-247` dapat commit payer lalu gagal pada recipient/journal tanpa compensation otomatis. `@Transactional` tidak membuat call lintas adapter atomic. | Gunakan saga/outbox dengan reversal dan durable reconciliation per leg; crash/failure test harus membuktikan tidak ada debit tanpa pasangan credit atau status recovery. |
...
| PROD-040 | P0 | Billing retry side effects | `billing-service/.../PaymentService.java:44-131,134-214` memakai `@Retry`, membuat payment/reference baru pada setiap invocation, memanggil biller/provider sebelum event publish, dan hanya mengandalkan release reservation; kegagalan release dicatat sebagai manual intervention. Retry setelah provider sukses atau event gagal dapat membuat side effect/reference kedua. | Persist operation idempotency key dan reference sebelum external call, reuse pada retry, lalu durable compensation/reconciliation untuk wallet, biller, dan outbox; uji crash di setiap boundary. |
