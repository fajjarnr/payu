...
| GRPC-005 | 🟠 | statement | `TransactionServiceClient` REST **tidak deprecated** — **PARTIAL 2026-08-12**: (GRPC-008) kontrak di-align + (layering) client di `adapter.client`, DTO di `statement.dto`; **prasyarat GRPC-002 SUDAH ADA** (server transaction gRPC live 9090). Sisa: tulis client gRPC `TransactionServiceGrpc` di statement + fallback REST + migrasi StatementService/ReceiptService — item sesi berikutnya | TransactionServiceClient.java; TransactionGrpcService |
