...
| OPS-2026-08-01-05 | P2 | Chaos | Kraken/preprod gate runtime: cerberus FIX (kubeconfig_path di config YAML — L-185, "client set"). Sisa: chaos pod kena mutasi `set-readonly-root-filesystem` (readOnlyRootFilesystem=true) walau exclusion chaos-engineering → krkn report `Read-only file system`. Investigate kenapa exclude mutasi tak berlaku (cache/selector). | 🔒 Investigate |
