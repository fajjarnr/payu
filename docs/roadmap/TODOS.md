...
| KAFKA-QUORUM-001 | P2 | **FIXED 2026-09-12 (akar: NetworkPolicy, bukan quorum)** — `allow-kafka-platform` (egress) blackhole DNS khusus pod label-strimzi (bukti diferensial: pod-IP OK vs ClusterIP FAIL, pod fresh tanpa label OK, dev TANPA policy ini sehat) → broker tak resolve voter → `FATAL caught up` 25 jam+. Fix live: hapus policy di sit/uat/preprod (+backup `/tmp/allow-kafka-*-backup.yaml`) → SIT 1 broker + 3 controller (scale dari 1, samakan dev) + UAT/preprod 1+1 SEMUA 1/1, CR Ready. Wipe PVC SIT hanya simtomatik (volume fresh ikut fatal sebelum DNS pulih). **RECURRENCE CLOSED 2026-09-18 (git)**: sumber git `infrastructure/platform/messaging/overlays/common/network-policy.yaml` kini egress `- {}` di `allow-kafka-platform`/`allow-amq-platform`/`allow-kafka-console-platform` (ingress zero-trust utuh) + `spec.egress.0…` replacement dihapus di sit/uat/preprod/prod + guard `test_messaging_kafka_policies_allow_unrestricted_egress` + `test_dev_messaging_has_no_kafka_egress_policy` 2/2 OK → Argo selfHeal tak bisa kembalikan policy jahat; live sync pending verifikasi cluster payu (reachable cluster tanpa ns `payu-*`) | `payu-kafka-broker-0` 228 restart; `getent`…
...
| RELAY-004 | P2 | **Full-reload buang sesi** — `tab.goto` ke `/transfer` mental ke `/login`, SPA aman. Store zustand in-memory hilang saat reload. | relay `NOFORM:/login?callbackUrl=%2Ftransfer` | CLOSED — rantai fix 1.18.86 (loopback rehidrasi) + 1.18.87 (Lax) + 1.18.99 (single-flight) + 1.18.100 (Secure); regression `proxy-auth` rehidrasi-expired→200 + Set-Cookie (bukan login), live no-cookie→307 login+callback; tanpa persist store (token tetap httpOnly, PCI-DSS) → `CHANGELOG.md` `1.18.100` |
| RELAY-005 | P3 | **Live realm drift** — cuma `customer1`, atribut phone/nik hilang (user-profile declarative drop unmanaged attrs). | `GET /admin/realms/payu/users` 1 user | CLOSED — live sync via Admin REST: profile `unmanagedAttributePolicy=None→ENABLED` (`/users/profile`, bukan realm — PUT realm 400), `customer1/2` attrs+roles per git, `admin`+`backoffice` created 201 per `keycloak-realm-import.yaml`; `probe1@x.id` asing dibiarkan; direct-grant login tak dipakai verifikasi (PKCE-only, tak diubah) → `CHANGELOG.md` `1.18.100` |
...
| RELAY-010 | P2 | **Seed job dev permanen** — wallet/account/seed + realm users hanya manual; dev baru = journey mati lagi. | Temuan sesi ini | CLOSED — Flyway permanen: wallet `V124` (5 wallet, ON CONFLICT, tanpa ledger) + account `V113` (users/profiles/accounts link KC sub, ON CONFLICT); live `payu-dev` teraplikasi sbg role `payu` (wallets 5 exact, users 3 link, re-run 0/0/0) + runbook `docs/operations/runbooks/dev-seed.md` (realm re-sync flow) → `CHANGELOG.md` `1.18.100` |
| RELAY-011 | P1 | **gRPC deadline beku di shared stub** — `withDeadlineAfter` saat init → semua call 30 dtk pasca-boot `DEADLINE_EXCEEDED` offset negatif membesar (semua service pemakai helper). | `-321s` di log, 0 call sampai wallet | CLOSED — transaction per-call + remainder 1.18.98: billing/fx/investment/promotion/statement `WalletGrpcAdapter` + lending `AccountGrpcClient` + statement `TransactionServiceClient` per-call via `interceptedStub`, test 5/5, image `:1.18.98` 6/6 Running → `CHANGELOG.md` `1.18.98` |
...
| Key | Pri | Temuan | Bukti | Status |
|---|---|---|---|---|
| FE-AUDIT-001 | P2 | **Race refresh ganda client+server** — `lib/api.ts:76` (401 → refresh, queued browser-side) vs `[...path]/route.ts:313` (tiap proxied request 401 → refresh server-side) pakai cookie refresh single-use yang sama → N-1 rotasi gagal → logout. | audit AuthBffAudit F1 | CLOSED 1.18.99 — single-flight di `POST /api/auth/refresh` (`inflightRotations` key cookie), test coalesce 6→1 + independen sesi, vitest 79/79 + web-app `:1.18.99` live → `CHANGELOG.md` `1.18.99` |
| FE-AUDIT-002 | P3 | **Derivasi flag Secure cookie tak konsisten** — refresh/logout dari env `NEXT_PUBLIC_BASE_URL` vs callback/authorize dari request proto (x-forwarded-proto aware) → flap bila env=http di belakang LB https. | audit AuthBffAudit F2 | CLOSED 1.18.100 — `POST /api/auth/refresh` + `POST /api/auth/logout` kini request-derived (`x-forwarded-proto` → URL proto → env fallback), test proto-vs-env, vitest 6/6 + 83/83 auth + `tsc` 0, web-app `:1.18.100` live OpenShift root 200 + `/api/health` 200 + refresh 401 `Secure` → `CHANGELOG.md` `1.18.100` |
...
| FE-AUDIT-004 | P3 | **Input `type=number` + helper float tanpa caller hidup** — pockets/split-bill amount `type=number` (e/exponent), `validation.ts parseIndonesianAmount/validateAmount` float, `currency.ts` compact/`parseCurrency`/`roundCurrency` legacy. Display-only/laten. | audit MoneyBoundaryAudit M3–M5 | CLOSED 1.18.101 — float helpers tanpa caller produksi dihapus (`parseCurrency`/`isValidCurrency`/`roundCurrency`, `validateAmount`/`parseIndonesianAmount` + test pengunci); semua page amount sudah `parseCurrencyExact`; `type=number` dibiarkan (input invalid terdegradasi ke reject via exact-parse, bukan salah nominal) → `CHANGELOG.md` `1.18.101` |
...
| FE-AUDIT-006 | P2 | **Analytics backend nol padahal transaksi ada** — akar: outbox tak pernah drain (bean mati) + consumer bind `None` + event tanpa identitas. CLOSED sisi pipa (drain 68+40, replay, `user_id` di event baru). Sisa: event lama keyed account-id; wallet event tanpa userId. | proof relay + API probe + Kafka lag 0 | CLOSED 1.18.102/1.18.103 — keying terbukti accountId di semua sisi (event `user_id`=accountId, WS sudah accountId; frontend query sub→accountId di dashboard+analytics, test key-identity 2/2); consumer crash `transaction_type` NameError + metrics-drop first-tx diperbaiki (11/11); wallet `change_amount/type` dari producer (9 situs) + recipient-resolve number→UUID via port gRPC; gateway schema terima string kanonis. Live journey customer1→customer2 Rp15.000 COMPLETED + double-entry exact + agregasi terisi (COMPLETED row, metrics 1/15000, wallet DEBIT/CREDIT 15000) + REST accountId→data vs sub→null → `CHANGELOG.md` `1.18.102`/`1.18.103` |
| FE-AUDIT-007 | P2 | **User Keycloak tanpa baris User account-service** — `isAccountOwner` warn "no user found for externalId=07f1aecf…" → semua endpoint ownership-gated 403 (budgets). Frontend wiring benar (403 → empty state). | account-service log + API probe | CLOSED via RELAY-010 — `V113` seed users (`external_id` = live KC sub) + profiles + accounts (`1001001001/2001/9001`); PII plaintext read-safe (`ENC()` passthrough); live link terbukti `users⋈accounts` 3/3 → `CHANGELOG.md` `1.18.100` |

### Audit 2026-09-11 — Identity Drift SIT/UAT (register 500, per-service loop account-service)

> Temuan saat validasi fix k6 register: dev `201`, SIT/UAT `500 Identity provider unavailable`. Rantai: account → gateway `/api/v1/auth/register` → auth-service `keycloakAdmin` → Keycloak master `admin-cli`.

| Key | Pri | Temuan | Bukti | Status |
|---|---|---|---|---|
| IDN-001 | P1 | **UAT auth → Keycloak mati ganda** — base `PAYU_KEYCLOAK_SERVER_URL` hardcode `payu-sso` (namespace kosong) + `PAYU_KEYCLOAK_ADMIN_USERNAME` hardcode `admin` (master cuma punya `keycloak-admin`) → `ProcessingException` + `user_not_found`. | `AuthController`/`auth-service` log UAT, `payu-keycloak` events master, curl token `keycloak-admin`+secret = `access_token` valid | CLOSED — root overlay UAT `PAYU_KEYCLOAK_SERVER_URL → payu-uat` + base username → `valueFrom payu-keycloak-admin/username` (dev `admin` tak berubah perilaku); register UAT `201 {userId,…}` live → `CHANGELOG.md` Unreleased |
| IDN-003 | P1 | **Gateway UAT JWKS ke `payu-sso` mati** — `QUARKUS_OIDC_AUTH_SERVER_URL` base `payu-sso` tanpa override UAT → `UnknownHostException`, JWT processor tak pernah init, semua token ditolak. Ditemukan saat selidiki kenapa login tetap gagal pasca-register `201`. | log gateway `Failed to initialize JWT processor`, render root buktikan 0 `payu-sso` di `QUARKUS_OIDC_AUTH_SERVER_URL` | CLOSED — `AUTH_SERVER_URL → payu-uat` di 3 blok Quarkus UAT (api-portal/gateway/notification) cermin SIT; rollout gateway sukses, tanpa error JWKS |
| IDN-002 | P1 | **SIT master admin password drift** — master punya `admin` tapi `invalid_user_credentials` untuk password secret live maupun git (`payu-sit-kc-admin-2026`); `keycloak-admin`+secret juga gagal. Tak ada jalur GitOps-only (bootstrap cuma saat DB kosong). | curl token grant 3 kombinasi → `invalid_grant` semua; secret 15:10:00 vs STS 15:10:17 tak menjelaskan | OPEN — butuh persetujuan: reset kredensial master via prosedur darurat DB Keycloak, atau re-bootstrap identity SIT. Dampak: register + `k6-load-test` SIT tetap merah (deploy/gate lain hijau) |
| KAFKA-UAT-001 | P1 | **Broker UAT CrashLoop: DNS gagal khusus dari pod Strimzi** — broker-0 `UnknownHostException payu-kafka-controller-1…` 40×; controller-1 sehat (leader, ~300 events/min). Terbukti BUKAN: DNS server (pod lain se-node resolve instan), TCP (pod-IP reachable), sertifikat (valid s/d 2027), nsswitch/resolv (identik). Klaim lama "BUKAN policy" GUGUR 2026-09-18: policy git `allow-kafka-platform` egress `:53`-gated terbukti blackhole DNS pod-Strimzi di SIT (KAFKA-QUORUM-001, signature identik: pod-IP OK vs ClusterIP FAIL, dev-tanpa-policy sehat); eliminasi lain tetap valid. Fix kandidat SAMA sudah di git (`network-policy.yaml` egress `- {}` + guard 2/2 OK, semua overlay render EXIT 0) — verifikasi live butuh cluster payu (reachable cluster tanpa ns `payu-*`; Argo sync pending). OPEN sampai broker UAT 1/1 pasca-sync atau bukti `getent` baru menyanggah | log JVM `UnknownHostException` + `getent` per-pod matrix + cert valid + endpoint ada + render sit/uat/preprod/prod EXIT 0 |
