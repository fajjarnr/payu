...
|:---|:---:|:---|:---|
| ACCOUNT-006 | P1 | Coverage account ~21% line/19% branch; integration test tidak required di CI. **PROGRESS 2026-08-13**: core domain 55.2%→95.6%; gate-facing coverage (excl generated `grpc`/dto/entity/config/domain) 24.7%→50.9% — HealthIndicatorTest (4), CachedAccountQueryServiceTest (8), BudgetServiceTest (11), AccountSecurityServiceTest (6), AccountWebControllersTest (6), UserPersistenceAdapterTest (7) + jacoco check exclude generated `**/grpc/**`. CI workflow ada. Sisa: gate-facing ≥80% (butuh ~490 baris lagi — Budget/Beneficiary controller, AccountPersistenceAdapter, health web), jacoco `verify` merah sampai naik. Done: ≥80% overall, 100% core domain, required CI. | 🟠 Test gate insufficient |
...
| QAMVP-010 | transaction, loan-origination | ~~Disbursement integration + E2E; wajib setelah ARCH-LOAN-001 fix~~ **integration DONE 2026-08-13** — `DisbursementServiceIntegrationTest` (2: create→process→complete + bankReference persisted; idempotency key dedup → 1 row, real PG Testcontainers). ARCH-LOAN-001 sudah fix (persistNew). Sisa: E2E blackbox | Disbursement journey hijau |
...
| QAMVP-016 | Coverage: jacoco `check` goal TER-BIND di `verify` (account 0.80); CI workflow account/backend mengeksekusi test + report (`.github/workflows/account-tests.yml` artifact). Sisa: account overall masih ~21% (core domain 95.6%), kyc 65% < 80%, Makefile `clean-test` hapus jacoco.exec (wajar) | 🟠 |
...
| QAMVP-019 | Frontend: halaman statement **DIBUAT 2026-08-13** (`/statements` page + `StatementsPage.test.tsx` 3 test, 104/104 pages suite green, tsc clean). Sisa: budget E2E, a11y filter (color-contrast/button-name), refresh-token expiry E2E, forgot-password/not-found stub | 🟡 |
