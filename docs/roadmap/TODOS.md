...
| PROD-022 | P0 | Loan repayment money movement | Repayment kini menjadi command durable: `loan_repayment_payments` (Flyway V9) menyimpan state/idempotency, schedule memakai row-lock + unique `(loan_id, installment_number)`, wallet melakukan debit + balanced `1100/1500` journal, dan hasil dipublish via outbox CloudEvent. Amount positif/scale ≤4, overpay, replay, duplicate schedule, timeout reconciliation, dan retry diuji. Image lending `1.8.101` + wallet `1.8.100` live; authenticated money E2E dengan fixture finansial terisolasi masih open. | Jalankan authenticated money E2E pada fixture finansial terisolasi untuk membuktikan debit, ledger, outbox, replay, dan reconciliation di cluster. |
