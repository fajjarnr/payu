...
| **Last Updated** | 2026-08-01 (OPS-2026-08-01-03/04 resolved — root cause kyverno default-deny + OVN egress, fix L-188; kraken gate pending capacity; DR restore verified) |
...
| MVP-001 | P1 | `SnapBiPaymentService.createPayment` (endpoint SNAP-BI `POST /payment`) TIDAK menyentuh mesin uang: hanya simpan record `PENDING` + return 200 "Successful" — tanpa debit/reserve wallet, tanpa escrow/hold, tanpa transisi status `COMPLETED` berbasis pergerakan dana, dan webhook keluar (`sendWebhookNotification`/`sendRefundWebhookNotification`) cuma stub `LOG.info`. Mesin uang nyata (wallet escrow/journal/settlement, `TransferSagaOrchestrator` via gRPC, QR settlement REST `/debit`) ada & matang TAPI terpisah dari jalur SNAP payment. Dampak: integrator dapat 200 padahal dana tidak pernah pindah. Fix: wire `createPayment` ke escrow yang sama dipakai `InitiateTransferCommandHandler`/`EscrowService`, publish event outbox `payment.completed`, dan gunakan `WebhookDispatcherService` yang sudah ada (HMAC+retry) — bukan stub `LOG.info`. Referensi: `SnapBiPaymentService.java` (konstruktor hanya 2 repo), `EscrowService.java`, `WebhookDispatcherService.java`. (Catatan: `TransferSagaOrchestrator` bukan target wiring — lihat MVP-002.) | 🔴 Gap confirmed — design wiring pending |
...
| MVP-003 | P1 | VA `POST /api/v1/payments/va/callback` (`VirtualAccountService.handleBankCallback`) menandai VA `markPaid` + save, tapi **tidak kredit wallet merchant, tidak publish event, tidak punya wallet/event port** (grep `VirtualAccountService.java` kosong utk wallet/credit/publish). Dana koleksi VA "masuk" tapi tak pernah sampai ke ledger/wallet siapapun → uang hilang dari sistem. `creditBalance` di transaction-service hanya dipanggil `InitiateTransferCommandHandler` (kredit penerima transfer) — bukan utk VA. Fix: `handleBankCallback` harus kredit wallet merchant/partner (via `WalletServicePort.creditBalance`) + publish event outbox `payment.completed`, dgn idempotensi callback. | 🔴 Gap confirmed — wiring credit pending |
| MVP-004 | P1 | Idempotency GAPS pada jalur money & callback eksternal (aturan non-negotiable: semua endpoint payment/transfer wajib `X-Idempotency-Key`): (1) SNAP-BI `POST /v1/partner/payments` (SnapBiController:148) & `POST /payments/{id}/refund` (:275) **tanpa `@Idempotent`** — keamanan lain sudah OK (timestamp replay-window + Bearer + JWS), tapi tak ada idempotensi & tak ada unique-constraint `partnerReferenceNo` → double-submit bikin duplikat; refund tanpa idempotency + cek cumulative sum tak dalok → race double-refund saat di-wire. (2) `POST /api/v1/disbursements/callback` (DisbursementController:132) & `POST /api/v1/payments/va/callback` (VirtualAccountController:70) **tanpa `@Idempotent` DAN tanpa `@PreAuthorize`** — callback eksternal bisa di-forge/dobel (VA callback terkait MVP-003). Fix: tambah `@Idempotent(required=true)` pada 4 endpoint tsb + unique-constraint `partnerReferenceNo` + auth/validasi callback. | 🟡 Partial — SNAP natural-key idempotency done (guard V15/V16); VA/disbursement callbacks masih pending (idempotency+auth) |
...
### 🔴 MVP-001: SNAP-BI `createPayment` tidak menyentuh mesin uang (money-flow gap)
* **Trace (2026-08-01)**: `SnapBiController` → `SnapBiPaymentService.createPayment()` (baris 41-79) = validasi amount > 0 → buat `SnapBiPaymentEntity` status `PENDING` → `paymentRepository.save()` → return 200 "Successful". Konstruktor `SnapBiPaymentService` hanya inject 2 repo (payment + refund) — **tanpa** wallet client/grpc/escrow.
* **Yang tidak terjadi di jalur ini**: debit/reserve wallet, pembuatan escrow/hold, transisi `PENDING→COMPLETED` oleh settlement nyata, dan webhook keluar (`sendWebhookNotification` baris 222-224 hanya `LOG.info` — stub, tidak kirim HTTP).
* **Mesin uang yang benar-benar ter-wire (tapi terpisah dari jalur SNAP payment)**: `InitiateTransferCommandHandler` (reserve→commit→credit + outbox + compensation) di transaction-service; wallet escrow/journal/settlement (HOLD/RELEASE/SETTLE/REFUND, akun 1100/2100/2200); `MerchantService.settleToMerchantWallet` (QR, REST `/debit`) di partner-service; `PaymentWebhookHandler` (inbound BI-FAST/QRIS). (Catatan: `TransferSagaOrchestrator` sering dirujuk sebagai engine, tapi ia dead code — lihat MVP-002.)
* **Dampak MVP**: integrator `POST /payment` dapat 200 padahal dana tidak berpindah — alur uang inti belum siap rilis.
* **Fix (murah — infrastruktur sudah ada)**: wire `createPayment` → escrow (`EscrowService`/`WalletServicePort`) + publish event outbox `payment.completed`. Untuk webhook: **jangan buat baru** — `WebhookDispatcherService` (HMAC + retry backoff + retry terjadwal) sudah matang dan dipakai `MerchantService`/`PaymentLinkService`/consumer; `SnapBiPaymentService` tinggal inject & `webhookDispatcher.dispatch("payment.completed", ...)`. `updatePaymentStatus()` (baris 114-131) juga perlu memanggil dispatcher, bukan stub. Verifikasi end-to-end: build Java tak bisa dijalankan di sandbox (transport Maven rusak) — perlu env build sehat / `oc` untuk bukti live.
...
### 🔴 MVP-003: VA `bankCallback` tidak kredit wallet merchant (funds-hilang path)
* **Trace (2026-08-01)**: `POST /api/v1/payments/va/callback` → `VirtualAccountService.handleBankCallback` (baris 95-118) = validasi VA pending + cocokkan amount → `va.markPaid(amount, ref)` → `save`. **Tidak**: kredit wallet merchant/partner, publish event, atau punya wallet/event port (grep `VirtualAccountService.java` kosong utk wallet/credit/publish).
* **Bukti kredit hanya 1 situs**: `grep creditBalance` di transaction-service → hanya `InitiateTransferCommandHandler.java:208` (kredit penerima transfer internal). VA collection tidak pernah menkredit siapapun.
* **Dampak MVP**: pembeli bayar via VA (callback bank "PAID"), tapi merchant tak pernah dana/ledger → uang hilang dari sistem (collection diakui, atribusi nol).
* **Fix**: `handleBankCallback` wajib kredit wallet merchant/partner via `WalletServicePort.creditBalance` + publish event outbox `payment.completed`, dengan idempotensi callback (VA reference). 

### 🔴 MVP-004: Idempotency gaps — SNAP payment/refund + callback eksternal
...
* **Gap 1 — SNAP-BI (SnapBiController)**: `POST /payments` (:148) & `POST /payments/{id}/refund` (:275) **tanpa `@Idempotent`**. Keamanan lain OK: replay-window timestamp (:169), Bearer+clientId (:173-188), JWS signature (:192). Tapi tanpa idempotency & tanpa unique-constraint `partnerReferenceNo` → double-submit bikin 2 record PENDING. Refund tanpa idempotency + cek `sumRefundedAmount` non-atomic → race double-refund saat didanai.
* **Gap 2 — callback eksternal**: `POST /api/v1/disbursements/callback` (DisbursementController:132) & `POST /api/v1/payments/va/callback` (VirtualAccountController:70) — **tanpa `@Idempotent` & tanpa `@PreAuthorize`**. Callback bank/BI-FAST bisa di-forge atau dobel (VA terkait MVP-003: tak boleh dobel markPaid/credit).
* **Fix**: `@Idempotent(required=true)` pada 4 endpoint tsb + unique-constraint `partnerReferenceNo` di `snap_bi_payment`, + autentikasi/validasi HMAC callback.
