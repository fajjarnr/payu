...
| QAMVP-016 | Coverage: jacoco `check` goal TER-BIND di `verify` — account **GATE GREEN 2026-08-13** (80.1% gate-facing, ACCOUNT-006 closed); CI workflow account/backend/kyc ada. Sisa: kyc coverage gate (65% < 80% — butuh run CI + test tambahan), Makefile `clean-test` hapus jacoco.exec (wajar) | 🟡 |
...
| QAMVP-019 | Frontend: statement page, forgot-password (3), not-found page + test (2) DIBUAT 2026-08-13; **a11y color-contrast/button-name SUDAH di-test** (`__tests__/a11y/color-contrast.test.tsx` + `screen-reader.test.tsx`, 49/49) dan refresh-token expiry di-test (`useSilentRefresh.test.ts`) — finding stale. **Full suite 94 files / 1210 test green**. Sisa minor: budget E2E spec, WCAG-strict tuning a11y filter | 🟡 |
