...
Catatan sesi 2026-08-26: audit Playwright headless (chromium 1366x900, locale id) — `/login` 200 + tombol SSO tampil; `/onboarding` 200 + stepper tampil; 401 `/api/auth/refresh` (normal, belum login); pod `payu-dev` 49/49 Running 1/1, SSO `sso-dev` realm `payu` well-known 200 — infrastruktur sehat, murni defect konfigurasi/hidrasi web-app. Audit route terautentikasi (20 route) tertahan sampai login fix (WEB-CSP-001).
