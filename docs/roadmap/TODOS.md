...
| QAMVP-003 | billing, partner | ~~Contract test: billing payment, SNAP-BI payment/refund/auth-token + CloudEvents contract~~ **CloudEvents contract DONE 2026-08-13** — `CloudEventsContractTest` (outbox-starter): record terbit bawa header `ce-specversion=1.0.2` + ce-id/source/type/time, topic `payu.<domain>.<event>.v<n>`, payload envelope `specversion:1.0.2`. Sisa: SNAP-BI payment/refund/auth-token contracts | `tests/contract` bertambah |
