...
| QAMVP-011 | wallet, transaction, billing, partner | ~~Test idempotency concurrency: 10 thread key sama → 1 mutasi, 1 ledger, 1 outbox~~ **wallet DONE 2026-08-13** — `WalletControllerConcurrencyIdempotencyTest`: 10 thread X-Idempotency-Key sama → 1 mutasi (use-case invocation), 1 successful atomic claim, 0 5xx; interceptor + IdempotencyService REAL (in-memory thread-safe repo), body-caching + response-caching filter direplikasi. Sisa: transaction, billing, partner (pola sama) | Test thread lulus CI |
...
| Key | Temuan | Sev |
|:---|:---|:---:|
| QAMVP-011 | Idempotency concurrency: wallet **FIXED 2026-08-13** (`WalletControllerConcurrencyIdempotencyTest`, 10 thread → 1 mutasi); transaction/billing/partner masih 0 | 🟡 |
