...
| FE-AUDIT-007 | P2 | **User Keycloak tanpa baris User account-service** — `isAccountOwner` warn "no user found for externalId=07f1aecf…" → semua endpoint ownership-gated 403 (budgets). Frontend wiring benar (403 → empty state). | account-service log + API probe | OPEN — seed User (+link akun) via registrasi/seed job, jangan hand-insert |
