...
| ARCH-TOPIC-003 | 🟠 | wallet, transaction, billing | Consumer pakai topic off-standard: `fx-rates-updated` (default, bukan `payu.fx.rates-updated.v1`), `disbursement-batch`; orphan consumer billing `payu.billing.subscription-due.v1` tanpa publisher | **wallet DONE 2026-08-15** — 3 consumer (fx/refund/user-created) disinkronkan ke topic `payu.*.v1` + unwrap CE envelope + StringDeserializer, verified live; sisa: transaction `disbursement-batch` (publisher tidak ada), billing orphan | FxRateEventConsumer.java; WalletEventConsumer.java |
