...
| **Last Updated** | 2026-08-03 (MVP-003 VA settlement, PROD-004 analytics money, PROD-005 analytics replay dedup, PROD-006 analytics schema/API, PROD-007 integration placeholder removed, PROD-008 durable gateway config, PROD-009 shared money API, PROD-010 web auth/rendering, PROD-011 web auth rate limiting, PROD-012 web BFF resilience, PROD-013 web money precision, PROD-014 web probes/release, PROD-015 web dependency security, PROD-017 gateway observability, PROD-019 refund authorization, PROD-020 dispute IDOR, PROD-001 refund source-of-truth leg, and PROD-003 loan approval completed; PROD-021 durable refund command/idempotency deployed, downstream reversal-ledger executor/reconciliation still open; PROD-002 FX stub isolation updated with real provider/freshness/audit still open; OPS-2026-08-01-03/04 resolved — root cause kyverno default-deny + OVN egress, fix L-188; kraken gate pending capacity; DR restore verified) |
...
|---|---|---|---|---|
| PROD-001 | P0 | Dispute/refund | Source-of-truth leg fixed; refund creation now applies a persisted active-refund cumulative guard and emits a durable reversal command. Downstream reversal-ledger execution and reconciliation remain tracked with PROD-021. | Integrasikan port read-only ke transaksi asli, ambil amount/currency tervalidasi, lalu tuntaskan reversal-ledger executor dan reconciliation test. |
...
| ID | Pri | Area | Bukti | Minimum done |
|---|---|---|---|---|
| PROD-021 | P0 | Dispute refund execution | `REFUND_CUSTOMER/PARTIAL_REFUND` kini membuat refund melalui `RefundUseCase`, resolve wajib idempotency key, active refund totals menolak over-refund, dan `outbox_events` menyimpan `RefundRequested` dengan publisher retry. Consumer yang mengeksekusi reversal ke ledger serta reconciliation test masih belum ada. | Tambahkan consumer/executor idempotent yang melakukan reversal ledger dan persist reference/status, lalu uji reconciliation serta retry/crash recovery. |
