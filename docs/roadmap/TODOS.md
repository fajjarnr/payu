...
| **Backlog Aktif** | 3 tickets + 28 action items (CB-*/PROD-*/READY-*/DEVSECOPS-*) + gates partner/platform (2026-08-12) |
| **Last Updated** | 2026-08-12 (CB-026+028+018 + ACCOUNT-005 + PROD-046 closed; Active Tickets urut P0→P1) |
...
| DISPUTE-002 | 🟠 | dispute | DisputeService persistence defects (ter-expose oleh fix infra test, 2026-08-12): (a) `DisputePersistenceAdapter.save` membangun entity baru dengan id existing → `@Version` membuat `save()` persist → `EntityExistsException` saat update dispute dalam 1 request; (b) syncing `evidenceList` via replace collection → orphan-removal error; (c) reject → 409 | DisputePersistenceAdapter.save, DisputeControllerIntegrationTest 3 red |
