...
| **Last Release** | `1.10.63` (2026-08-12) |
...
| **Backlog Aktif** | 3 tickets + 19 action items (CB-*/PROD-*/READY-*/DEVSECOPS-*) + gates partner/platform (2026-08-12) |
| **Last Updated** | 2026-08-12 (CB-008/011/017/022/024/025/031/036 closed — seluruh P2 backlog aksi selesai; podman stack live 37 containers, image semver `1.10.63`) |
...
| GRPC-004 | 🟠 | transaction | `PaymentExpiryScheduler` panggil wallet-service **raw `RestTemplate` langsung ke URL** (bukan via port/adapter) — bypass hexagonal boundary + REST sync | PaymentExpiryScheduler.java:59,120-126 |
| GRPC-005 | 🟠 | statement | `TransactionServiceClient` REST **tidak deprecated** (tidak ada gRPC alternatif, karena GRPC-002 server-nya juga belum ada) — statement satu-satunya service yang belum migrasi gRPC sama sekali | TransactionServiceClient.java:14,19 |
...
| GRPC-020 | 🟡 | wallet | Validasi scale 4 tidak konsisten di jalur gRPC: `transfer` (WalletService.java:438,517) enforce `scale() > 4`, tapi `reserveBalance` (152) & `credit` (336) hanya cek `amount <= 0` tanpa scale check → amount `"0.00001"` lolos reserve/credit | WalletService.java:152,336 vs 438,517 |
| GRPC-021 | 🟡 | shared | **Tidak ada enforcement rule**: (a) ArchUnit rule pelarangan `RestTemplate`/`WebClient` di adapter client antar-service TIDAK ADA di service manapun; (b) grpc-starter **0 test** (tidak ada src/test); (c) akar kenapa GRPC-003..008 bisa drift tanpa ketahuan | grep noClasses.*RestTemplate = 0 hit; shared/grpc-starter/src/test = 0 file |
...
| WALLET-001 | 🔴 | wallet | **Runtime bug transaksional (verified live, 2026-08-12)**: `GetBalance` gRPC → `PSQLException: Cannot commit when autoCommit is true` (WalletGrpcService.java:52 → WalletService.getBalance tx proxy). Dugaan akar: `datasource-starter` bind prefix `spring.datasource.primary.hikari.*` (DataSourceAutoConfiguration.java) vs `application-local.yml` taruh jdbc-url di `spring.datasource.primary.*` (tanpa `.hikari`) → properti auto-commit tidak ter-bind ke pool yang dipakai EMF | WalletGrpcService.java:52; DataSourceAutoConfiguration.java:40-47; wallet4.log:9430 |
| WALLET-002 | 🔴 | wallet | **Migration gap (verified live)**: app TIDAK BISA boot dengan schema migrasi — `split_payment_legs.settled_at` tidak pernah ada di migrasi (V10 tanpa kolom, tidak ada ALTER) + `split_recipients.recipient_type` beda nama dgn kolom `type` di V10 (entity @Column("recipient_type") vs DDL `type`) → Hibernate validation fail. Butuh migrasi baru V113+ | SplitPaymentLegEntity.java:41; SplitRecipientEntity.java:30; V10__create_split_payment_tables.sql:65-75 |
| GRPC-012 | 🔴 | wallet | **Proto `Debit` salah implementasi**: `WalletGrpcService.debit()` memanggil `walletService.reserveBalance()` (bukan debit domain) → balance **tidak berkurang**, hanya di-reserve; caller dapat `success=true` + reservationId sebagai transactionId. Gateway bridge `/api/internal/grpc/wallet/debit` meneruskan Debit ini — uang path semantics salah | WalletGrpcService.java:127-131; WalletGrpcBridge.java:82-87 |
...
| GRPC-008 | 🔴 | lending/statement | **Kontrak REST internal sudah drift — bukti kenapa sync query wajib gRPC (proto versioned)**: (a) `TransactionServiceClient` statement panggil `/api/v1/transactions/customer/{customerId}` — endpoint tidak ada di transaction-service (404; yang ada `GET /api/v1/transactions?accountId=`); (b) `TransactionClient` lending panggil `/user/{userId}`, `/account/{accountId}`, `/user/{userId}/summary` — tidak ada (hanya `/accounts/{accountId}` plural); (c) `AccountClient` lending panggil `/api/v1/users/{userId}` + `/external/{externalId}` — account-service tidak punya controller `/api/v1/users`, gateway tidak punya route `users` → 404 di semua jalur | TransactionServiceClient.java:30,58; TransactionClient.java:16-22; AccountClient.java:13,16; TransactionController.java:90 |
