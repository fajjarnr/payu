...
| Open Bugs (TODOS.md)     | 🟢 0 P0, 0 P1, 0 P2 (all CLOSED)         | **iter-73 (Jul 2)**: P2 audit sweep — closed AUDIT-037/040/041/043/044/045/046/047/050/051/056/057/058/061/062/063/064, reclassified AUDIT-055→P3, AUDIT-039/060→deferred. 0 P0, 1 P1, 0 P2 open. |
...
| Last Status Update       | 2026-07-02                               | **Completed iter-73**: P2 audit sweep. Decoupled configurations for all 17 Spring Boot services. Upgraded Next.js to 16.2.9 with security patch audit fixes. Parameterized test database and keycloak admin credentials. Audited and closed gateway idempotency, outbox regex, and allowed origins. |
...
> ✅ **iter-73 — P2 Audit Sweep & Environment Decoupling (Jul 2, 2026)**: Closed 13 audit items. Decoupled environment configurations into standard pipelines: (1) Root `.env.{dev,sit,uat,preprod,prod}.example` templates; (2) Spring Boot profile configuration split (`application.yml` + `application-{local,dev,sit,uat,preprod,prod}.yml`) across all 17 Spring Boot services. Upgraded Next.js to `16.2.9` with `npm audit fix` security updates successfully verified via clean typecheck, lint, and production build. Parameterized all test passwords in dispute/auth services test configs. Audited and closed gateway idempotency, outbox regex, and allowed origins. Deferred mobile/use-client items. Board summary: 0 P0, 1 P1, 0 P2.
