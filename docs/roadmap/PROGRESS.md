...
|:-------------------------|:-----------------------------------------|:------------------------------------------------|
| Services Deployed        | 🟢 28/28 Running                        | All 23 backend services + 4 simulators + web-app — all 1/1 Ready, no CrashLoopBackOff. SSO (Keycloak), Kafka, PostgreSQL HA (Crunchy PG 3-node), Infinispan (DataGrid 8.6), Artemis AMQ (2-node active/passive) — all connected. |
| Total Pods               | 🟢 45/45                                | 28 app + 5 PostgreSQL + 5 Kafka + 2 Artemis + 1 DataGrid + 1 config-listener + 3 Kafka controller/entity-operator |
| OpenShift Cluster        | 🟢 Active                                | OCP 4.21, 6 nodes (3 master + 3 worker), ap-southeast-1 AWS. Cluster API: api.payu.ocp.fajjjar.my.id:6443 |
| Operators Installed      | 🟡 16/17                                 | Tekton, ArgoCD, RHBK (Keycloak 26), AMQ Streams, AMQ Broker, DataGrid, cert-manager, External Secrets, Compliance, ACS, ServiceMesh, Kiali, 3scale (2.13 — 2.16 not in catalog), KubeDescheduler, Crunchy PG. Missing: RHDH (not in marketplace). |
| Data Services            | 🟢 All running                            | Crunchy PG17 HA 3-node. DataGrid 8.6 single-node (Infinispan). AMQ Streams (Kafka) 5-broker + 3-controller KRaft. Artemis 2-node active/passive. |
| Identity (Keycloak)      | 🟢 Deployed, reachable                    | RHBK 26 operator in payu-sso. Realm `payu` + OIDC clients configured. All services connect via OIDC_ISSUER. |
| Maven Build              | 🟢 41/41                                 | BUILD SUCCESS all modules. |
| Images Pushed            | 🟢 28/28                                | `:1.8.81` / `:1.8.82` pushed via podman to internal registry. |
| Cert-Manager             | 🟡 ClusterIssuer ready, cert pending     | LE production DNS-01 via Route53 — pending valid AWS IAM credentials. |
| CI/CD                    | 🟡 ArgoCD + Tekton installed            | No pipelines or GitOps sync configured yet. |
| **Production Readiness** | 🟡 85/100                                | All services + infra running. Missing: WAF, Observability stack (Tempo/Loki/Prometheus), CI/CD pipelines, DR runbook, Vault auto-unseal. |
| Last Status Update       | 2026-07-03                               | **Cluster fully stable**. 45/45 pods Ready. v1.8.82 deployed. All infra connected. |
