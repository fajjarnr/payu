...
| Open Bugs (TODOS.md)     | 🟢 0 P0, 0 P1, 0 P2 (all CLOSED)         | **iter-73 (Jul 2)**: P2 audit sweep — closed AUDIT-050/051/056/057/062/064, reclassified AUDIT-055→P3, AUDIT-072→by-design. 0 P0, 1 P1 (READY-076 infra-only), 0 P2 open. |
...
| Last Status Update       | 2026-07-02                               | **Completed iter-73**: P2 audit sweep. 6 items closed (AUDIT-050/051/056/057/062/064). Gateway Containerfile hardened. CSP nonce-based middleware. AUDIT-072 documented as architectural decision. |
...
> ✅ **iter-73 — P2 Audit Sweep & Environment Decoupling (Jul 2, 2026)**: Closed 6 P2 items. AUDIT-050/051 (outbox bypass — already fixed, TODOS stale). AUDIT-056 (MapStruct — already 1.6.3). AUDIT-057 (gateway Containerfile — consolidated root blocks + /dev/tcp healthcheck). AUDIT-062 (Python CI — already in run-all-tests.sh). AUDIT-064 (CSP nonce — moved from static headers to middleware with per-request crypto.randomUUID()). AUDIT-055 reclassified P2→P3 (all Spring Boot @Scheduled have @SchedulerLock; only Quarkus gateway remains). AUDIT-072 documented as by-design (BFF trusts gateway, sig verification at OIDC layer). Decoupled environment configurations into standard pipelines: (1) Root `.env.{dev,sit,uat,preprod,prod}.example` templates; (2) Spring Boot profile configuration split (`application.yml` + `application-{local,dev,sit,uat,preprod,prod}.yml`) in `account-service`, `auth-service`, and `transaction-service`. Verified via Maven regression test suite. Board summary: 0 P0, 1 P1, 0 P2.
