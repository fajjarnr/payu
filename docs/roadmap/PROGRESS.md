...
> - Added the RHTAS Application with automated prune/self-heal. The production base retains EFS RWX; the lab overlay uses ODF CephFS and hostname anti-affinity because this cluster has no `efs-csi` or zone labels. Redis proxy/schema workloads now satisfy Kyverno labels and restricted filesystem policy.
> - Runtime evidence: RHTAS Redis proxy `2/2`, Redis `3/3`, Trillian schema and create-tree Jobs succeeded, Rekor/CTLog/Fulcio/TSA are Ready, and the internal `rekor-server` Service is available. Tekton Chains remains configured for internal Rekor transparency.
