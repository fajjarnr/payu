...
- **Verification**: gateway/web-app/sso-sit `200`, 6/6 rollout kunci, PKCE journey Rp15.000 string → `201 COMPLETED` (`TXN-CC4716CC5EF04AA2`) + double-entry exact (DEBIT/CREDIT 15000.0000). Kafka broker crashloop saat storm 118 topik (self-healed setelah idle; L-440). HPA `lending-service-hpa` tanpa `apiVersion` (pre-existing, inert di dev juga; KEDA pegang scaling). Root `replicas count:1` tak efektif live (pre-existing; live 2-3 sehat).
- **Known gaps**: V114 hanya di git (dev juga 113 — paritas exact, ikut build berikut); `scripts/tools.sh` dirty pre-existing tak tersentuh; wallet 1 pod Pending spread (node penuh, non-blocking).
