...
- **Flyway Duplicate Fix (TXN-HARDEN-003/004 → B3.1)**: `backend/transaction-service/src/main/resources/db/migration/V29__force_row_level_security.sql` duplicate `Found more than one migration with version 29` (`V29 inbox_events` + `V29 force RLS` both added `e01cf79c` B3) → `V30__force_row_level_security.sql` rename `V29→V30` + header `V30` + `payu_transaction.flyway_schema_history` `V28` last success `V29 inbox` pending `V30 force` now sequential, `oc exec payu-database-1 psql payu_transaction SELECT version` `V28` → `V29/V30` next deploy `transaction-service 1.18.14` `CrashLoopBackOff 52 → 1/1 Running` `0 ERROR` via `oc logs --previous`.
- **SemVer Sync 1.18.14 (31 workloads + 31 pipelines + 31 pipelineRuns + podman-compose)**: `package.json 1.18.13→1.18.14` `podman-compose.yml 31× PAYU_VERSION 1.18.13→1.18.14` `per-service pipelines 31× image-tag 1.18.13→1.18.14` `pipelineRuns 31× 1.18.13→1.18.14` `workloads/overlays/payu-dev/kustomization.yaml 30× newTag 1.18.9→1.18.14` `workloads/overlays/payu-dev/*/kustomization.yaml 31× newTag 1.18.8/1.18.9/1.18.11→1.18.14` `workloads sit/uat/preprod/prod 124× 1.18.14`, `oc tag -n payu-dev payu-dev/<svc>:<latest> payu-dev/<svc>:1.18.14` `31/31` (`account 1.18.11, analytics 1.18.8, transaction 1.18.10→1.18.14` etc), `oc kustomize payu-dev | grep image: 1.18.14` `31/31`, `oc apply -k payu-dev` `30 configured` + `oc kustomize account-service | oc apply -f -` `account 1.18.11→1.18.14`, `oc get deployment -n payu-dev -o json | jq image 31 1.18.14`, `oc get is -n payu-dev 31 1.18.14` `40/44 1/1 → 44/44 1/1` after ArgoCD `hard refresh` `payu-dev OutOfSync d99858aa → Synced`.
- **KEDA 2.14 Operator + ScaledObjects Applied (KEDA-HARDEN-001 → B4.6)**: `helm repo add kedacore` `helm upgrade --install keda kedacore/keda --namespace keda --create-namespace --version 2.14.0 --set replicaCount=2` `keda-operator 1/1 Running 2/2` `keda-operator-metrics-apiserver 1/1` `keda-admission-webhooks 1/1` `oc apply -k infrastructure/platform/keda/base` `TriggerAuthentication payu-kafka-auth (ExternalSecret payu-kafka-credentials Vault payu/prod/kafka)` `ScaledObject transaction-service-keda min3 max10 lag10 kafka payu.transaction.initiated.v1 prometheus 1000 QPS polling 15s cooldown 30s fallback 3` `wallet-service-keda` `gateway-service-keda` `biller/va-sim keda min0 max3 lag5` `5 ScaledObjects` `oc get scaledobject -n payu-dev 5` `True/Unknown` `oc get hpa` KEDA behind, `overlays/prod` fixed `wallet prometheus payu-prod` + `va/biller bootstrap payu-prod` (KedaVerify defects 1+2).
- **Verification 1.18.14**: `oc get pods -n payu-dev 44/44 1/1` (`payu-dev` 31 apps + 5 simulators + `payu-database 3/3 2/2` `payu-cache 1/1` `payu-broker-ss 2/2` `payu-kafka 6/6` `coraza-waf 2/2` `keda 3/3`) `oc logs --since=60s -n payu-dev --all-containers | grep -E '"level":"ERROR"|"level":"WARN"' 0` (`transaction CrashLoop 52` now `1/1`, `web-app ImagePullBackOff` now `1.18.14`), `oc get pipelinerun -n payu-cicd transaction-service-build-fl9tg Running 7m` `compile` `BUILD SUCCESS` pending `k6 threshold` `argocd-sync` `gitops-writeback`, `oc get cluster 5/5 Healthy` `oc get kafka 6/6` `oc get certificate 5/5 True`.
- **Backlog**: `TODOS No OPEN P1` `HARDEN ponytail ceilings` reviewed (TXN/ACC/COMPLIANCE/GATEWAY/PORTAL remain deferred with ceiling, KEDA applied) `CHAOS-ENV-001` `SX-AUTH-001` `SSO-ENV-002` deferred pending platform creds (DEVSECOPS-017), `LLM DEFERRED` `TODOS` `LESSONS L-344` `TAGS v1.18.14`.
