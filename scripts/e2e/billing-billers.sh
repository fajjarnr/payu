...
echo
echo "========== Biller Catalog (Read-only) =========="

T1=$(run_test "T1: List billers (GET /api/v1/billers)" \
    "$BASE/api/v1/billers$UP" \
    -H "Authorization: Bearer $JWT")
assert_http "T1 biller list" "200" "$T1"
assert_json_exists "T1 data" "data"

T2=$(run_test "T2: List biller categories (GET /api/v1/billers/categories)" \
    "$BASE/api/v1/billers/categories$UP" \
    -H "Authorization: Bearer $JWT")
assert_http "T2 categories" "200" "$T2"

T3=$(run_test "T3: List topup providers (GET /api/v1/topup/providers)" \
    "$BASE/api/v1/topup/providers$UP" \
    -H "Authorization: Bearer $JWT")
assert_http "T3 providers" "200" "$T3"

echo
echo "========== Error Flows =========="

T4=$(run_test "T4: Nonexistent biller (GET /api/v1/billers/UNKNOWN)" \
    "$BASE/api/v1/billers/UNKNOWN$UP" \
    -H "Authorization: Bearer $JWT")
assert_http "T4 not found" "404" "$T4"

T5=$(run_test "T5: No JWT (401)" \
    "$BASE/api/v1/billers$UP")
assert_http "T5 no JWT" "401" "$T5"

T6=$(run_test "T6: Invalid JWT (401)" \
    "$BASE/api/v1/billers/categories$UP" \
    -H "Authorization: Bearer dead.token")
