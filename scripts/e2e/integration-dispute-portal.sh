...
echo "=== Integration ==="
T1=$(t "T1: Status" "$INT/api/v1/integration/status" -H "Authorization: Bearer $JWT"); ok "T1" "200" "$T1"
T2=$(t "T2: Messages" "$INT/api/v1/integration/messages" -H "Authorization: Bearer $JWT"); ok "T2" "200" "$T2"
T3=$(t "T3: No JWT" "$INT/api/v1/integration/status"); ok "T3" "401" "$T3"

echo "=== Dispute ==="
T4=$(t "T4: List" "$DSP/api/v1/disputes" -H "Authorization: Bearer $JWT"); ok "T4" "200" "$T4"
T5=$(t "T5: By status" "$DSP/api/v1/disputes/status/OPEN" -H "Authorization: Bearer $JWT"); ok "T5" "200" "$T5"
T6=$(t "T6: No JWT" "$DSP/api/v1/disputes"); ok "T6" "401" "$T6"
