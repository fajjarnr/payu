...
class TestLivenessService:
    ...
    def valid_face_image(self):
        ...
        img[100:300, 200:400] = np.random.randint(100, 200, (200, 200, 3), dtype=np.uint8)
        return img
    ...
    def test_detect_face_selects_largest_face(self, liveness_service):
        ...
        with patch('cv2.CascadeClassifier') as mock_cascade:
            ...
            detected, face_box = liveness_service._detect_face(np.zeros((480, 640, 3), dtype=np.uint8))
    ...
    async def test_check_liveness_below_threshold(self, liveness_service):
        ...
        with patch('cv2.imdecode', return_value=img):
            with patch.object(liveness_service, '_detect_face', return_value=(True, (0, 0, 100, 100))):
                with patch.object(liveness_service, '_calculate_liveness_score', return_value=0.5):
                    result = await liveness_service.check_liveness(b"low_liveness_image")
    ...
    async def test_check_liveness_above_threshold(self, liveness_service):
        ...
        with patch('cv2.imdecode', return_value=img):
            with patch.object(liveness_service, '_detect_face', return_value=(True, (0, 0, 200, 200))):
                with patch.object(liveness_service, '_calculate_liveness_score', return_value=0.8):
                    result = await liveness_service.check_liveness(b"high_liveness_image")
    ...
    def test_liveness_feature_weights(self, liveness_service):
        ...
        with patch.object(liveness_service, '_extract_liveness_features',
                         return_value={
                             'eye_openness': 1.0,
                             'mouth_movement': 1.0,
                             'head_pose': 1.0,
    ...
    async def test_check_liveness_returns_details(self, liveness_service):
        ...
        with patch('cv2.imdecode', return_value=img):
            with patch.object(liveness_service, '_detect_face', return_value=(True, (0, 0, 200, 200))):
                with patch.object(liveness_service, '_assess_face_quality', return_value=0.75):
                    with patch.object(liveness_service, '_calculate_liveness_score', return_value=0.85):
