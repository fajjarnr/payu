...
async def test_store_uses_datagrid_rest_json_contract():
    ...
    async with httpx.AsyncClient(
        ...
        assert await store.get_cached_result("key", "/v1/kyc") is None
        await store.store_result("key", "/v1/kyc", {"status": "ok"})
...
async def test_store_round_trips_through_local_datagrid_rest():
    ...
    try:
        key = str(uuid.uuid4())
        result = {"status": "ok", "source": "kyc"}
        await store.store_result(key, "/v1/kyc", result)
        assert await store.get_cached_result(key, "/v1/kyc") == result
    finally:
