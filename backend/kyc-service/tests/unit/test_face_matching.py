...
class TestFaceService:
    ...
    def ktp_image_with_face(self):
        ...
        img[50:150, 100:200] = np.random.randint(100, 200, (100, 100, 3), dtype=np.uint8)
        return img
    ...
    @pytest.mark.asyncio
    async def test_match_face_both_faces_detected(self, face_service, ktp_image_with_face, selfie_image_with_face):
        ...
        with patch('cv2.imread', return_value=ktp_image_with_face):
            with patch('cv2.imdecode', return_value=selfie_image_with_face):
                with patch('cv2.CascadeClassifier') as mock_cascade:
                    ...
                    mock_instance.detectMultiScale.side_effect = [mock_faces_ktp, mock_faces_selfie]
    ...
    @pytest.mark.asyncio
    async def test_match_face_ktp_image_not_found(self, face_service, selfie_image_with_face):
    ...
    def test_detect_face_selects_largest(self, face_service):
        ...
        with patch('cv2.CascadeClassifier') as mock_cascade:
            ...
            detected, face_box = face_service._detect_face(np.zeros((480, 640, 3), dtype=np.uint8))
    ...
    @pytest.mark.asyncio
    async def test_match_face_similarity_above_threshold(self, face_service, ktp_image_with_face):
