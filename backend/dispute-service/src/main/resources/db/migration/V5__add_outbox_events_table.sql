-- Transactional outbox for durable refund commands.
CREATE TABLE IF NOT EXISTS outbox_events (
    ...
    CONSTRAINT chk_outbox_aggregate_id_not_empty CHECK (LENGTH(TRIM(aggregate_id)) > 0),
    CONSTRAINT chk_outbox_event_type_not_empty CHECK (LENGTH(TRIM(event_type)) > 0),
    CONSTRAINT chk_outbox_retry_count_non_negative CHECK (retry_count >= 0)
);
