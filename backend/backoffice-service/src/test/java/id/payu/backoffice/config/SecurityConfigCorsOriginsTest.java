...
/**
 * AUDIT-053 fix: backoffice-service {@link SecurityConfig#corsConfigurationSource()}
...
class SecurityConfigCorsOriginsTest {
    ...
    @Test
    void valueDefaultExpressionShouldPreserveBackofficeProductionOrigins() throws Exception {
        Field field = SecurityConfig.class.getDeclaredField("allowedOrigins");
        Value annotation = field.getAnnotation(Value.class);

        assertThat(annotation.value())
            .contains("https://backoffice.payu.fajjjar.my.id")
            .contains("https://backoffice.payu.co.id")
            .contains("https://admin.payu.fajjjar.my.id");
    }
