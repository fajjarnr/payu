     ...
     <div className="max-w-xl w-full relative z-10">
      ...
      <div className="mb-12 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 bg-bank-green/10 px-4 py-2 rounded-full border border-bank-green/20">
         ...
         <span className="text-[10px] font-black tracking-[0.2em] text-bank-green">Protokol Identitas</span>
        </div>
        <div className="w-12" />
      </div>
      ...
      <div className="flex justify-between items-center mb-16 relative px-4">
        ...
        {[1, 2, 3].map((s) => (
         <div key={s} className="relative z-10 flex flex-col items-center">
           <div className={`w-14 h-14 rounded-xl flex items-center justify-center font-black text-xl transition-all duration-700 shadow-2xl ${step >= s ? 'bg-bank-green text-white scale-110 shadow-bank-green/30' : 'bg-card text-gray-300 border border-border'}`}>
            {s}
           </div>
         </div>
        ))}
      ...
      <div className="bg-card/80 backdrop-blur-xl rounded-xl p-10 border border-border shadow-2xl min-h-[450px] flex flex-col justify-center relative overflow-hidden group">
        ...
        {step === 1 && (
         <div className="space-y-12 animate-in slide-in-from-right duration-700">
           <div className="text-center">
            <h2 className="text-4xl font-black text-foreground mb-4">Verifikasi eKYC.</h2>
           ...
           <div className="aspect-video bg-gray-50 dark:bg-gray-900/50 rounded-xl flex flex-col items-center justify-center border-2 border-dashed border-border cursor-pointer hover:border-bank-green hover:bg-bank-green/5 transition-all duration-500 group/upload">
            ...
            <span className="text-[10px] font-black text-gray-400 group-hover/upload:text-bank-green tracking-[0.2em]">Ambil Gambar Identitas</span>
           </div>

           <button
            onClick={() => setStep(2)}
            className="w-full bg-bank-green text-white py-6 rounded-xl font-black text-[10px] tracking-[0.2em] hover:bg-bank-emerald transition-all active:scale-[0.98] shadow-2xl shadow-bank-green/20 flex items-center justify-center gap-3"
           >
            Mulai Proses Verifikasi <ChevronRight className="h-5 w-5" />
           </button>
           <div className="flex items-center justify-center gap-3 text-[10px] font-black text-gray-400 tracking-widest bg-gray-50 dark:bg-gray-900/50 py-3 rounded-xl border border-border">
        ...
        {step === 2 && (
         <form onSubmit={handleSubmit(onSubmit)} className="space-y-12 animate-in slide-in-from-right duration-700">
           <div className="text-center mb-4">
            <h2 className="text-4xl font-black text-foreground mb-3">Profil Akun.</h2>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="group">
              <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 block mb-3 group-focus-within:text-bank-green transition-colors">Nomor NIK (16 Digit)</label>
              <input
               {...register('nik')}
               type="text"
               ...
               placeholder="3200..."
              />
              {errors.nik && <p className="text-red-500 text-[10px] mt-3 pl-3 font-black tracking-widest">{errors.nik.message}</p>}
            </div>

            <div className="group">
              <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 block mb-3 group-focus-within:text-bank-green transition-colors">Nama Lengkap Sesuai KTP</label>
              <input
               {...register('fullName')}
               type="text"
               className="w-full rounded-xl border-border bg-gray-50 dark:bg-gray-900/50 p-5 text-foreground font-bold placeholder:text-gray-200 focus:ring-4 focus:ring-bank-green/10 focus:border-bank-green transition-all outline-none"
              ...
              />
              {errors.fullName && <p className="text-red-500 text-[10px] mt-3 pl-3 font-black tracking-widest">{errors.fullName.message}</p>}
            </div>

            <div className="group">
              <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 block mb-3 group-focus-within:text-bank-green transition-colors">Alamat Email Digital</label>
              <input
               {...register('email')}
               type="email"
               className="w-full rounded-xl border-border bg-gray-50 dark:bg-gray-900/50 p-5 text-foreground font-bold placeholder:text-gray-200 focus:ring-4 focus:ring-bank-green/10 focus:border-bank-green transition-all outline-none"
              ...
              />
              {errors.email && <p className="text-red-500 text-[10px] mt-3 pl-3 font-black tracking-widest">{errors.email.message}</p>}
            </div>

            <div className="group">
              <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 block mb-3 group-focus-within:text-bank-green transition-colors">Nama Pengguna (Username)</label>
              <input
               {...register('username')}
               type="text"
               className="w-full rounded-xl border-border bg-gray-50 dark:bg-gray-900/50 p-5 text-foreground font-bold placeholder:text-gray-200 focus:ring-4 focus:ring-bank-green/10 focus:border-bank-green transition-all outline-none"
           ...
           <div className="pt-6">
            <button
              ...
              className="w-full bg-bank-green text-white py-6 rounded-xl font-black text-[10px] tracking-[0.2em] hover:bg-bank-emerald transition-all active:scale-[0.98] shadow-2xl shadow-bank-green/20"
            >
        ...
        {step === 3 && (
         <div className="flex flex-col items-center justify-center py-16 animate-in zoom-in duration-700 text-center relative z-10">
           ...
           <h2 className="text-5xl font-black text-foreground mb-4">Pendaftaran Berhasil.</h2>
