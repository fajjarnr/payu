...
export default function BillsPage() {
  ...
  if (selectedBiller) {
    return (
      <DashboardLayout>
        <div className="max-w-2xl mx-auto space-y-8 sm:space-y-10 pb-10">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-foreground uppercase tracking-tight italic">Bayar {selectedBiller.name}</h2>
          </div>
          ...
          <div className="bg-card rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-sm border border-border relative overflow-hidden group">
            ...
            <div className="relative z-10 flex items-center gap-4 sm:gap-6 mb-6 sm:mb-10 pb-6 sm:pb-10 border-b border-border">
              <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] ${selectedBiller.color} flex items-center justify-center shadow-xl transition-transform group-hover:scale-110`}>
              ...
              <div>
                ...
                <h3 className="text-xl sm:text-2xl font-black text-foreground uppercase tracking-tight">{selectedBiller.name}</h3>
            ...
            <div className="space-y-6 sm:space-y-10 relative z-10">
              <div className="group">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border-border bg-gray-50 dark:bg-gray-900/50 p-4 sm:p-6 text-base sm:text-xl font-black text-foreground placeholder:text-gray-200 focus:ring-4 focus:ring-bank-green/10 focus:border-bank-green transition-all outline-none uppercase tracking-tight"
                />
              </div>
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full rounded-2xl border-border bg-gray-50 dark:bg-gray-900/50 p-4 sm:p-6 pl-14 sm:pl-16 text-2xl sm:text-3xl font-black text-foreground placeholder:text-gray-200 focus:ring-4 focus:ring-bank-green/10 focus:border-bank-green transition-all outline-none tracking-tighter"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-6">
            <button
              ...
              className="w-full bg-bank-green text-white py-5 sm:py-6 rounded-[1.25rem] sm:rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-bank-emerald transition-all active:scale-[0.98] shadow-2xl shadow-bank-green/20 disabled:bg-bank-green/50"
            >
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl mx-auto space-y-10 sm:space-y-12 pb-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground uppercase tracking-tight italic">Tagihan & Top-up</h2>
        ...
        {/* Biller Grid */}
        <div className="bg-card rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-sm border border-border relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 sm:w-64 h-48 sm:h-64 bg-bank-green/5 rounded-full blur-3xl" />
          <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6 sm:mb-10 text-center">Kategori Layanan</h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-4 sm:gap-10 relative z-10">
            {billers.map((item) => (
              ...
              >
                <div className={`w-14 h-14 sm:w-20 sm:h-20 rounded-[1.25rem] sm:rounded-[1.5rem] ${item.color} flex items-center justify-center shadow-lg shadow-black/5 group-hover:scale-110 group-hover:shadow-xl group-hover:border group-hover:border-white/20 transition-all duration-300`}>
                  <item.icon className="h-6 w-6 sm:h-8 sm:w-8" />
                </div>
                <span className="text-[9px] sm:text-[10px] font-black text-foreground uppercase tracking-widest">{item.name}</span>
              </button>
            ))}
            <button className="flex flex-col items-center gap-3 sm:gap-5 transition-all group active:scale-95">
              <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-[1.25rem] sm:rounded-[1.5rem] bg-gray-50 dark:bg-gray-900 shadow-inner flex items-center justify-center text-gray-300 group-hover:text-bank-green transition-colors">
                <Plus className="h-6 w-6 sm:h-8 sm:w-8" />
              </div>
              <span className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">Lainnya</span>
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6 sm:space-y-8">
          <h3 className="text-lg sm:text-xl font-black text-foreground uppercase tracking-tight italic">Aktivitas Terakhir</h3>
          {!isLoading && recentBills.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              {recentBills.map((bill: PaymentResponse) => (
                <div key={bill.id} className="bg-card p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] flex items-center justify-between border border-border hover:shadow-xl transition-all group">
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-bank-green/10 flex items-center justify-center text-bank-green transition-transform group-hover:scale-110">
                      <Zap className="h-5 w-5 sm:h-6 sm:w-6" />
                    </div>
                    <div>
                      <div className="font-black text-foreground text-xs sm:text-sm uppercase tracking-tight">{bill.billerCode}</div>
                  ...
                  <div className="text-right">
                    <div className="font-black text-foreground text-sm sm:text-base italic">Rp {bill.amount.toLocaleString('id-ID')}</div>
            ...
            <div className="bg-card rounded-[2rem] sm:rounded-[3rem] p-10 sm:p-16 text-center border border-border border-dashed border-2 flex flex-col items-center justify-center">
