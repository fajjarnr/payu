...
- **E-15 — Payment Gateway Features (2026-02-27)**:
  ...
  - **IMP-044 — Payment Expiry & Auto-Cancel** (2 SP): Centralized payment expiry in `transaction-service`. `PaymentExpiryScheduler` with `@Scheduled` jobs every 5 minutes: expire pending transactions (sets CANCELLED + failure reason), expire pending VAs (marks EXPIRED). Added `expiresAt` field to `Transaction` entity. `TransactionJpaRepository.findExpiredPendingTransactions()` JPQL query.
  - **IMP-045 — Dynamic QR for Merchants** (5 SP): Merchant onboarding and dynamic QRIS in `partner-service`. Domain: `Merchant` entity (partner-scoped, merchantCode, businessName, category: FOOD_BEVERAGE/RETAIL/etc, status: PENDING_REVIEW→ACTIVE/SUSPENDED), `MerchantQrPayment` entity (dynamic QR per transaction, referenceId, QR content, status: PENDING→PAID/EXPIRED). `MerchantService` with onboarding, activation, dynamic QR generation (QRIS-format content), QR payment confirmation, `@Scheduled` QR expiry. `MerchantController` REST at `/merchants` (CRUD with @Audited/@Idempotent, QR generation, payment confirmation with payer-level auth). Flyway V6 migration (2 tables). 10+ unit tests.
...
- **E-11 — Subscription & Recurring Billing (2026-02-26)**:
  - **GAP-008 — Subscription / Recurring Billing** (5 SP): Full subscription lifecycle in `billing-service`. Domain: `SubscriptionPlan` (partner-defined plans with `BillingInterval` DAILY/WEEKLY/MONTHLY/YEARLY, pricing, trial days, grace period), `Subscription` (account-to-plan binding with `SubscriptionStatus` TRIAL/ACTIVE/PAST_DUE/SUSPENDED/CANCELLED, dunning tracking, period management), `SubscriptionCharge` (per-billing-cycle charge record with `ChargeStatus` PENDING/SUCCEEDED/FAILED/REFUNDED, idempotency key). `SubscriptionService` orchestrates: plan CRUD, subscribe with optional trial, `@Scheduled` recurring charge processor (every 5min), dunning retry with 3-attempt exhaustion→suspend, trial expiry→activate+charge, period advancing by interval. `SubscriptionController` REST at `/api/v1/subscriptions` with 10 endpoints (plan CRUD, subscribe, cancel, get by account/partner, charge history). `@EnableScheduling` added. Flyway V3 migration (3 tables with indexes). 22 unit tests passing.
