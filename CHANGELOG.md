...
- **QAMVP-014 (wallet, billing)**: security test vs real filter chain — `WalletSecurityTest` (unauth → 401, non-owner → 403, owner → 200; Boot 4 pakai `org.springframework.boot.webmvc.test.autoconfigure.AutoConfigureMockMvc` + `payu.grpc.server.port=0`), `PaymentSecurityTest` (401 + ownership).
