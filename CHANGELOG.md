...
- **Refund transaction source (PROD-001 source-of-truth leg)**: full refund kini mengambil amount/currency tervalidasi dari transaction-service melalui outbound port dan endpoint read-only ber-role operator; partial refund memastikan transaction ada, currency cocok, dan amount tidak melebihi transaksi. Duplicate-request/reversal-ledger execution tetap berada di PROD-021.
- **FX hardcoded stub isolation (PROD-002 partial)**: `StubFxRateProviderAdapter` kini hanya aktif pada profile `local`, unknown currency pair tidak fallback ke rate `1`, dan profile non-local fail-closed bila `realFxRateProvider` belum tersedia. Provider produksi, freshness/source validation, dan pair/rate audit tetap open.
...
> **Verify PROD-002 partial**: fx-service reactor tests 60/60 + package BUILD SUCCESS; image `1.8.84` digest `sha256:8f6f363d77219a0e07b515e0ce135a8e755cea0e603e12b85a7a6123b18749b6`, pod Ready 1/1, actuator health `UP`, active profiles `grpc,container`, no restart (2026-08-03). Explicit integration run remains blocked by pre-existing test wiring (`TestRestTemplate` bean absent; 9 errors); real provider/freshness/source/audit remains open.
