...
- **QAMVP-013 (wallet)**: `OutboxAtomicityIntegrationTest` — transactional outbox atomicity vs PostgreSQL real (Testcontainers di podman socket `DOCKER_HOST=unix:///run/user/1000/podman/podman.sock`): commit → business row (`refund_reversal_executions`) + outbox row sama-sama ada; rollback (`REQUIRES_NEW` + throw) → 0 keduanya. Bonus: `TransactionalPoolAutoCommitTest` + `SchemaMigrationIntegrityTest` (sebelumnya gagal tanpa Docker) kini lulus — wallet suite penuh 54/54 green. Repo starter tambah `findByAggregateId`.
