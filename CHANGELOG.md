...
- `transaction-service` 151 tests / 0 failures — new `PaymentExpirySchedulerTest` (2: conditional expire publishes once; a VA a callback paid is never overwritten) and `VirtualAccountServiceTest` rewrites (`shouldReturnExistingOnReplayedCallbackWithoutCredit`, `shouldSettleExactlyOnceAcrossTwoCallbacks`; the old "replayed callback throws" contract became a deterministic no-op per IMP-2).
